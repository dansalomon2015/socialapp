...
const CreatePostView = props => {
      ...
      return (
        <View style={styles.inputContainer}>
          <CsTextInput
            ...
            theme={theme}
          />
          <Image source={{ uri: file_path }} style={[styles.imageStyle, { borderRadius: 20 }]} resizeMode="cover" />
        </View>
      );
    case POST_TYPE_VIDEO:
      return (
  ...
  return (
    <SafeAreaView style={[sharedStyles.container, { backgroundColor: themes[theme].navbarBackground }]}>
      <StatusBar />
      <KeyboardView
        contentContainerStyle={[sharedStyles.contentContainer, { backgroundColor: themes[theme].backgroundColor, height: '100%' }]}
        keyboardVerticalOffset={128}>
        <ScrollView
          contentContainerStyle={{ flexGrow: 1, paddingTop: 10, paddingHorizontal: 10 }}
          {...scrollPersistTaps}>
          {isLoading && (
          ...
          <View style={styles.userContainer}>
            ...
            />
            <Text style={[styles.userName, { color: themes[theme].activeTintColor }]}>{user.displayName}</Text>
          </View>
          {renderForm()}
        </ScrollView>
      </KeyboardView>
