...
const HomeView = props => {
  ...
  const { followedPosts, followingPosts, loading, isUpdating, refreshing } = state
  ...
  const init = async () => {
    ...
    posts.forEach(doc => {
      ...
      if (!user.blocked || !user.blocked.includes(post.userId)) {
        if ((user.followers.includes(post.userId)) || user.userId === post.userId) {
  ...
  const renderScene = SceneMap({
    ...
    second: (() => <RenderFlatListItem type={'followings'} data={followingPosts} />),
  })
