import I18n from '../i18n';

export const POST_TYPE_TEXT = 'text';
export const POST_TYPE_PHOTO = 'photo';
export const POST_TYPE_VIDEO = 'video';

export const SITE_SHOP_URL = 'https://www.vipbillionaires.com';
export const SITE_VIP_MEMBERS_URL =
  'https://www.vipbillionaires.com/vip-members';
export const CLOUD_URL =
  'https://us-central1-vip-billionaires-c07df.cloudfunctions.net';
export const MERCHANT_ID = 'merchant.com.ninasoft.vipbillionaires';

export const Genders = [
  { value: 'male', text: I18n.t('Male') },
  { value: 'female', text: I18n.t('Female') },
  { value: 'none', text: I18n.t('Gender_other') },
];

export const Countries = [
  { value: 'Afghanistan', text: 'Afghanistan' },
  { value: 'Albania', text: 'Albania' },
  { value: 'Algeria', text: 'Algeria' },
  { value: 'Andorra', text: 'Andorra' },
  { value: 'Angola', text: 'Angola' },
  { value: 'Antigua and Barbuda', text: 'Antigua and Barbuda' },
  { value: 'Argentina', text: 'Argentina' },
  { value: 'Armenia', text: 'Armenia' },
  { value: 'Australia', text: 'Australia' },
  { value: 'Austria', text: 'Austria' },
  { value: 'Azerbaijan', text: 'Azerbaijan' },
  { value: 'Bahamas', text: 'Bahamas' },
  { value: 'Bahrain', text: 'Bahrain' },
  { value: 'Bangladesh', text: 'Bangladesh' },
  { value: 'Barbados', text: 'Barbados' },
  { value: 'Belarus', text: 'Belarus' },
  { value: 'Belgium', text: 'Belgium' },
  { value: 'Belize', text: 'Belize' },
  { value: 'Benin', text: 'Benin' },
  { value: 'Bhutan', text: 'Bhutan' },
  { value: 'Bolivia', text: 'Bolivia' },
  { value: 'Bosnia and Herzegovina', text: 'Bosnia and Herzegovina' },
  { value: 'Botswana', text: 'Botswana' },
  { value: 'Brazil', text: 'Brazil' },
  { value: 'Brunei', text: 'Brunei' },
  { value: 'Bulgaria', text: 'Bulgaria' },
  { value: 'Burkina Faso', text: 'Burkina Faso' },
  { value: 'Burundi', text: 'Burundi' },
  { value: 'Côte d\'Ivoire', text: 'Côte d\'Ivoire' },
  { value: 'Cabo Verde', text: 'Cabo Verde' },
  { value: 'Cambodia', text: 'Cambodia' },
  { value: 'Cameroon', text: 'Cameroon' },
  { value: 'Canada', text: 'Canada' },
  { value: 'Central African Republic', text: 'Central African Republic' },
  { value: 'Chad', text: 'Chad' },
  { value: 'Chile', text: 'Chile' },
  { value: 'China', text: 'China' },
  { value: 'Colombia', text: 'Colombia' },
  { value: 'Comoros', text: 'Comoros' },
  { value: 'Congo (Congo-Brazzaville)', text: 'Congo (Congo-Brazzaville)' },
  { value: 'Costa Rica', text: 'Costa Rica' },
  { value: 'Croatia', text: 'Croatia' },
  { value: 'Cuba', text: 'Cuba' },
  { value: 'Cyprus', text: 'Cyprus' },
  { value: 'Czechia (Czech Republic)', text: 'Czechia (Czech Republic)' },
  {
    value: 'Democratic Republic of the Congo',
    text: 'Democratic Republic of the Congo',
  },
  { value: 'Denmark', text: 'Denmark' },
  { value: 'Djibouti', text: 'Djibouti' },
  { value: 'Dominica', text: 'Dominica' },
  { value: 'Dominican Republic', text: 'Dominican Republic' },
  { value: 'Ecuador', text: 'Ecuador' },
  { value: 'Egypt', text: 'Egypt' },
  { value: 'El Salvador', text: 'El Salvador' },
  { value: 'Equatorial Guinea', text: 'Equatorial Guinea' },
  { value: 'Eritrea', text: 'Eritrea' },
  { value: 'Estonia', text: 'Estonia' },
  { value: 'Eswatini', text: 'Eswatini' },
  { value: 'Ethiopia', text: 'Ethiopia' },
  { value: 'Fiji', text: 'Fiji' },
  { value: 'Finland', text: 'Finland' },
  { value: 'France', text: 'France' },
  { value: 'Gabon', text: 'Gabon' },
  { value: 'Gambia', text: 'Gambia' },
  { value: 'Georgia', text: 'Georgia' },
  { value: 'Germany', text: 'Germany' },
  { value: 'Ghana', text: 'Ghana' },
  { value: 'Greece', text: 'Greece' },
  { value: 'Grenada', text: 'Grenada' },
  { value: 'Guatemala', text: 'Guatemala' },
  { value: 'Guinea', text: 'Guinea' },
  { value: 'Guinea-Bissau', text: 'Guinea-Bissau' },
  { value: 'Guyana', text: 'Guyana' },
  { value: 'Haiti', text: 'Haiti' },
  { value: 'Holy See', text: 'Holy See' },
  { value: 'Honduras', text: 'Honduras' },
  { value: 'Hungary', text: 'Hungary' },
  { value: 'Iceland', text: 'Iceland' },
  { value: 'India', text: 'India' },
  { value: 'Indonesia', text: 'Indonesia' },
  { value: 'Iran', text: 'Iran' },
  { value: 'Iraq', text: 'Iraq' },
  { value: 'Ireland', text: 'Ireland' },
  { value: 'Israel', text: 'Israel' },
  { value: 'Italy', text: 'Italy' },
  { value: 'Jamaica', text: 'Jamaica' },
  { value: 'Japan', text: 'Japan' },
  { value: 'Jordan', text: 'Jordan' },
  { value: 'Kazakhstan', text: 'Kazakhstan' },
  { value: 'Kenya', text: 'Kenya' },
  { value: 'Kiribati', text: 'Kiribati' },
  { value: 'Kuwait', text: 'Kuwait' },
  { value: 'Kyrgyzstan', text: 'Kyrgyzstan' },
  { value: 'Laos', text: 'Laos' },
  { value: 'Latvia', text: 'Latvia' },
  { value: 'Lebanon', text: 'Lebanon' },
  { value: 'Lesotho', text: 'Lesotho' },
  { value: 'Liberia', text: 'Liberia' },
  { value: 'Libya', text: 'Libya' },
  { value: 'Liechtenstein', text: 'Liechtenstein' },
  { value: 'Lithuania', text: 'Lithuania' },
  { value: 'Luxembourg', text: 'Luxembourg' },
  { value: 'Madagascar', text: 'Madagascar' },
  { value: 'Malawi', text: 'Malawi' },
  { value: 'Malaysia', text: 'Malaysia' },
  { value: 'Maldives', text: 'Maldives' },
  { value: 'Mali', text: 'Mali' },
  { value: 'Malta', text: 'Malta' },
  { value: 'Marshall Islands', text: 'Marshall Islands' },
  { value: 'Mauritania', text: 'Mauritania' },
  { value: 'Mauritius', text: 'Mauritius' },
  { value: 'Mexico', text: 'Mexico' },
  { value: 'Micronesia', text: 'Micronesia' },
  { value: 'Moldova', text: 'Moldova' },
  { value: 'Monaco', text: 'Monaco' },
  { value: 'Mongolia', text: 'Mongolia' },
  { value: 'Montenegro', text: 'Montenegro' },
  { value: 'Morocco', text: 'Morocco' },
  { value: 'Mozambique', text: 'Mozambique' },
  { value: 'Myanmar (formerly Burma)', text: 'Myanmar (formerly Burma)' },
  { value: 'Namibia', text: 'Namibia' },
  { value: 'Nauru', text: 'Nauru' },
  { value: 'Nepal', text: 'Nepal' },
  { value: 'Netherlands', text: 'Netherlands' },
  { value: 'New Zealand', text: 'New Zealand' },
  { value: 'Nicaragua', text: 'Nicaragua' },
  { value: 'Niger', text: 'Niger' },
  { value: 'Nigeria', text: 'Nigeria' },
  { value: 'North Korea', text: 'North Korea' },
  { value: 'North Macedonia', text: 'North Macedonia' },
  { value: 'Norway', text: 'Norway' },
  { value: 'Oman', text: 'Oman' },
  { value: 'Pakistan', text: 'Pakistan' },
  { value: 'Palau', text: 'Palau' },
  { value: 'Palestine State', text: 'Palestine State' },
  { value: 'Panama', text: 'Panama' },
  { value: 'Papua New Guinea', text: 'Papua New Guinea' },
  { value: 'Paraguay', text: 'Paraguay' },
  { value: 'Peru', text: 'Peru' },
  { value: 'Philippines', text: 'Philippines' },
  { value: 'Poland', text: 'Poland' },
  { value: 'Portugal', text: 'Portugal' },
  { value: 'Qatar', text: 'Qatar' },
  { value: 'Romania', text: 'Romania' },
  { value: 'Russia', text: 'Russia' },
  { value: 'Rwanda', text: 'Rwanda' },
  { value: 'Saint Kitts and Nevis', text: 'Saint Kitts and Nevis' },
  { value: 'Saint Lucia', text: 'Saint Lucia' },
  {
    value: 'Saint Vincent and the Grenadines',
    text: 'Saint Vincent and the Grenadines',
  },
  { value: 'Samoa', text: 'Samoa' },
  { value: 'San Marino', text: 'San Marino' },
  { value: 'Sao Tome and Principe', text: 'Sao Tome and Principe' },
  { value: 'Saudi Arabia', text: 'Saudi Arabia' },
  { value: 'Senegal', text: 'Senegal' },
  { value: 'Serbia', text: 'Serbia' },
  { value: 'Seychelles', text: 'Seychelles' },
  { value: 'Sierra Leone', text: 'Sierra Leone' },
  { value: 'Singapore', text: 'Singapore' },
  { value: 'Slovakia', text: 'Slovakia' },
  { value: 'Slovenia', text: 'Slovenia' },
  { value: 'Solomon Islands', text: 'Solomon Islands' },
  { value: 'Somalia', text: 'Somalia' },
  { value: 'South Africa', text: 'South Africa' },
  { value: 'South Korea', text: 'South Korea' },
  { value: 'South Sudan', text: 'South Sudan' },
  { value: 'Spain', text: 'Spain' },
  { value: 'Sri Lanka', text: 'Sri Lanka' },
  { value: 'Sudan', text: 'Sudan' },
  { value: 'Suriname', text: 'Suriname' },
  { value: 'Sweden', text: 'Sweden' },
  { value: 'Switzerland', text: 'Switzerland' },
  { value: 'Syria', text: 'Syria' },
  { value: 'Tajikistan', text: 'Tajikistan' },
  { value: 'Tanzania', text: 'Tanzania' },
  { value: 'Thailand', text: 'Thailand' },
  { value: 'Timor-Leste', text: 'Timor-Leste' },
  { value: 'Togo', text: 'Togo' },
  { value: 'Tonga', text: 'Tonga' },
  { value: 'Trinidad and Tobago', text: 'Trinidad and Tobago' },
  { value: 'Tunisia', text: 'Tunisia' },
  { value: 'Turkey', text: 'Turkey' },
  { value: 'Turkmenistan', text: 'Turkmenistan' },
  { value: 'Tuvalu', text: 'Tuvalu' },
  { value: 'Uganda', text: 'Uganda' },
  { value: 'Ukraine', text: 'Ukraine' },
  { value: 'United Arab Emirates', text: 'United Arab Emirates' },
  { value: 'United Kingdom', text: 'United Kingdom' },
  { value: 'United States of America', text: 'United States of America' },
  { value: 'Uruguay', text: 'Uruguay' },
  { value: 'Uzbekistan', text: 'Uzbekistan' },
  { value: 'Vanuatu', text: 'Vanuatu' },
  { value: 'Venezuela', text: 'Venezuela' },
  { value: 'Vietnam', text: 'Vietnam' },
  { value: 'Yemen', text: 'Yemen' },
  { value: 'Zambia', text: 'Zambia' },
  { value: 'Zimbabwe', text: 'Zimbabwe' },
];

export const CONTENT_PRIVACY_POLICY =
  '<h1>Privacy Policy</h1>\n' +
  '<p>Updated at 2021-09-27</p>\n' +
  '<p>VIP Billionaires (“we,” “our,” or “us”) is committed to protecting your privacy. This Privacy Policy explains how your personal\n' +
  'information is collected, used, and disclosed by VIP Billionaires.</p>\n' +
  '<p>This Privacy Policy applies to our website, and its associated subdomains (collectively, our “Service”) alongside our application,\n' +
  'VIP Billionaires. By accessing or using our Service, you signify that you have read, understood, and agree to our collection,\n' +
  'storage, use, and disclosure of your personal information as described in this Privacy Policy and our Terms of Service.</p>\n' +
  '<h3>Definitions and key terms</h3>\n' +
  '<p>To help explain things as clearly as possible in this Privacy Policy, every time any of these terms are referenced, are strictly\n' +
  'defined as:</p>\n' +
  '<p>● Cookie: small amount of data generated by a website and saved by your web browser. It is used to identify your browser,\n' +
  'provide analytics, remember information about you such as your language preference or login information.<br/>\n' +
  '● Company: when this policy mentions “Company,” “we,” “us,” or “our,” it refers to Zein El Dine International Co., Ltd, Tokyo,\n' +
  'Japan that is responsible for your information under this Privacy Policy.<br/>\n' +
  '● Country: where VIP Billionaires or the owners/founders of VIP Billionaires are based, in this case is Japan<br/>\n' +
  '● Customer: refers to the company, organization or person that signs up to use the VIP Billionaires Service to manage the\n' +
  'relationships with your consumers or service users.<br/>\n' +
  '● Device: any internet connected device such as a phone, tablet, computer or any other device that can be used to visit VIP\n' +
  'Billionaires and use the services.<br/>\n' +
  '● IP address: Every device connected to the Internet is assigned a number known as an Internet protocol (IP) address.\n' +
  'These numbers are usually assigned in geographic blocks. An IP address can often be used to identify the location from\n' +
  'which a device is connecting to the Internet.<br/>\n' +
  '● Personnel: refers to those individuals who are employed by VIP Billionaires or are under contract to perform a service on\n' +
  'behalf of one of the parties.<br/>\n' +
  '● Personal Data: any information that directly, indirectly, or in connection with other information — including a personal\n' +
  'identification number — allows for the identification or identifiability of a natural person.<br/>\n' +
  '● Service: refers to the service provided by VIP Billionaires as described in the relative terms (if available) and on this\n' +
  'platform.<br/>\n' +
  '● Third-party service: refers to advertisers, contest sponsors, promotional and marketing partners, and others who provide\n' +
  'our content or whose products or services we think may interest you.<br/>\n' +
  '● Website: VIP Billionaires."’s" site, which can be accessed via this URL:<br/>\n' +
  '● You: a person or entity that is registered with VIP Billionaires to use the Services.</p>\n' +
  '<h3>What Information Do We Collect?</h3>\n' +
  '<p>We collect information from you when you visit our app, register on our site, place an order, subscribe to our newsletter, respond\n' +
  'to a survey or fill out a form.</p>\n' +
  '<p>● Name / Username<br/>\n' +
  '● Phone Numbers<br/>\n' +
  '● Email Addresses<br/>\n' +
  '● Job Titles<br/>\n' +
  '● Billing Addresses<br/>\n' +
  '● Debit/credit card numbers<br/>\n' +
  '● Age<br/>\n' +
  '● Password</p>\n' +
  '<p>We also collect information from mobile devices for a better user experience, although these features are completely optional:</p>\n' +
  '<p>● Location (GPS): Location data helps to create an accurate representation of your interests, and this can be used to bring\n' +
  'more targeted and relevant ads to potential customers.<br/>\n' +
  '● Phonebook (Contacts list): Your contacts list allows the app to be much more easy to use by the user, since accessing\n' +
  'your contacts from the app makes you save tons of time.<br/>\n' +
  '● Camera (Pictures): Granting camera permission allows the user to upload any picture straight from the app, you can safely\n' +
  'deny camera permissions for this app.<br/>\n' +
  '● Photo Gallery (Pictures): Granting photo gallery access allows the user to upload any picture from their photo gallery, you\n' +
  'can safely deny photo gallery access for this app.</p>\n' +
  '<h3>How Do We Use The Information We Collect?</h3>\n' +
  '<p>Any of the information we collect from you may be used in one of the following ways:</p>\n' +
  '<p>● To personalize your experience (your information helps us to better respond to your individual needs)<br/>\n' +
  '● To improve our app (we continually strive to improve our app offerings based on the information and feedback we receive\n' +
  'from you)<br/>\n' +
  '● To improve customer service (your information helps us to more effectively respond to your customer service requests and\n' +
  'support needs)<br/>\n' +
  '● To process transactions<br/>\n' +
  '● To administer a contest, promotion, survey or other site feature<br/>\n' +
  '● To send periodic emails</p>\n' +
  '<h3>When does VIP Billionaires use end user information from third parties?</h3>\n' +
  '<p>VIP Billionaires will collect End User Data necessary to provide the VIP Billionaires services to our customers.</p>\n' +
  '<p>End users may voluntarily provide us with information they have made available on social media websites. If you provide us with\n' +
  'any such information, we may collect publicly available information from the social media websites you have indicated. You can\n' +
  'control how much of your information social media websites make public by visiting these websites and changing your privacy\n' +
  'settings.</p>\n' +
  '<h3>When does VIP Billionaires use customer information from third parties?</h3>\n' +
  '<p>We receive some information from the third parties when you contact us. For example, when you submit your email address to\n' +
  'us to show interest in becoming a VIP Billionaires customer, we receive information from a third party that provides automated\n' +
  'fraud detection services to VIP Billionaires. We also occasionally collect information that is made publicly available on social\n' +
  'media websites. You can control how much of your information social media websites make public by visiting these websites\n' +
  'and changing your privacy settings.</p>\n' +
  '<h3>Do we share the information we collect with third parties?</h3>\n' +
  '<p>We may share the information that we collect, both personal and non-personal, with third parties such as advertisers, contest\n' +
  'sponsors, promotional and marketing partners, and others who provide our content or whose products or services we think may\n' +
  'interest you. We may also share it with our current and future affiliated companies and business partners, and if we are involved\n' +
  'in a merger, asset sale or other business reorganization, we may also share or transfer your personal and non-personal\n' +
  'information to our successors-in-interest.</p>\n' +
  '<p>We may engage trusted third party service providers to perform functions and provide services to us, such as hosting and\n' +
  'maintaining our servers and the app, database storage and management, e-mail management, storage marketing, credit card\n' +
  'processing, customer service and fulfilling orders for products and services you may purchase through the app. We will likely\n' +
  'share your personal information, and possibly some non-personal information, with these third parties to enable them to perform\n' +
  'these services for us and for you.</p>\n' +
  '<p>We may share portions of our log file data, including IP addresses, for analytics purposes with third parties such as web\n' +
  'analytics partners, application developers, and ad networks. If your IP address is shared, it may be used to estimate general\n' +
  'location and other technographics such as connection speed, whether you have visited the app in a shared location, and type of\n' +
  'the device used to visit the app. They may aggregate information about our advertising and what you see on the app and then\n' +
  'provide auditing, research and reporting for us and our advertisers. We may also disclose personal and non-personal\n' +
  'information about you to government or law enforcement officials or private parties as we, in our sole discretion, believe\n' +
  'necessary or appropriate in order to respond to claims, legal process (including subpoenas), to protect our rights and interests\n' +
  'or those of a third party, the safety of the public or any person, to prevent or stop any illegal, unethical, or legally actionable\n' +
  'activity, or to otherwise comply with applicable court orders, laws, rules and regulations.</p>\n' +
  '<h3>Where and when is information collected from customers and end users?</h3>\n' +
  '<p>VIP Billionaires will collect personal information that you submit to us. We may also receive personal information about you from\n' +
  'third parties as described above.</p>\n' +
  '<h3>How Do We Use Your Email Address?</h3>\n' +
  '<p>By submitting your email address on this app, you agree to receive emails from us. You can cancel your participation in any of\n' +
  'these email lists at any time by clicking on the opt-out link or other unsubscribe option that is included in the respective email.\n' +
  'We only send emails to people who have authorized us to contact them, either directly, or through a third party. We do not send\n' +
  'unsolicited commercial emails, because we hate spam as much as you do. By submitting your email address, you also agree to\n' +
  'allow us to use your email address for customer audience targeting on sites like Facebook, where we display custom advertising\n' +
  'to specific people who have opted-in to receive communications from us. Email addresses submitted only through the order\n' +
  'processing page will be used for the sole purpose of sending you information and updates pertaining to your order. If, however,\n' +
  'you have provided the same email to us through another method, we may use it for any of the purposes stated in this Policy.\n' +
  'Note: If at any time you would like to unsubscribe from receiving future emails, we include detailed unsubscribe instructions at\n' +
  'the bottom of each email.</p>\n' +
  '<h3>How Long Do We Keep Your Information?</h3>\n' +
  '<p>We keep your information only so long as we need it to provide VIP Billionaires to you and fulfill the purposes described in this\n' +
  'policy. This is also the case for anyone that we share your information with and who carries out services on our behalf. When\n' +
  'we no longer need to use your information and there is no need for us to keep it to comply with our legal or regulatory\n' +
  'obligations, we’ll either remove it from our systems or depersonalize it so that we can\'t identify you.\n' +
  '<h3>How Do We Protect Your Information?</h3>\n' +
  '<p>We implement a variety of security measures to maintain the safety of your personal information when you place an order or\n' +
  'enter, submit, or access your personal information. We offer the use of a secure server. All supplied sensitive/credit information\n' +
  'is transmitted via Secure Socket Layer (SSL) technology and then encrypted into our Payment gateway providers database only\n' +
  'to be accessible by those authorized with special access rights to such systems, and are required to keep the information\n' +
  'confidential. After a transaction, your private information (credit cards, social security numbers, financials, etc.) is never kept on\n' +
  'file. We cannot, however, ensure or warrant the absolute security of any information you transmit to VIP Billionaires or\n' +
  'guarantee that your information on the Service may not be accessed, disclosed, altered, or destroyed by a breach of any of our\n' +
  'physical, technical, or managerial safeguards.\n' +
  '<h3>Could my information be transferred to other countries?</h3>\n' +
  '<p>VIP Billionaires is incorporated in Japan. Information collected via our website, through direct interactions with you, or from use\n' +
  'of our help services may be transferred from time to time to our offices or personnel, or to third parties, located throughout the\n' +
  'world, and may be viewed and hosted anywhere in the world, including countries that may not have laws of general applicability\n' +
  'regulating the use and transfer of such data. To the fullest extent allowed by applicable law, by using any of the above, you\n' +
  'voluntarily consent to the trans-border transfer and hosting of such information.\n' +
  '<h3>Is the information collected through the VIP Billionaires Service secure?</h3>\n' +
  '<p>We take precautions to protect the security of your information. We have physical, electronic, and managerial procedures to\n' +
  'help safeguard, prevent unauthorized access, maintain data security, and correctly use your information. However, neither\n' +
  'people nor security systems are foolproof, including encryption systems. In addition, people can commit intentional crimes,\n' +
  'make mistakes or fail to follow policies. Therefore, while we use reasonable efforts to protect your personal information, we\n' +
  'cannot guarantee its absolute security. If applicable law imposes any non-disclaimable duty to protect your personal information,\n' +
  'you agree that intentional misconduct will be the standards used to measure our compliance with that duty.\n' +
  '<h3>Can I update or correct my information?</h3>\n' +
  '<p>The rights you have to request updates or corrections to the information VIP Billionaires collects depend on your relationship\n' +
  'with VIP Billionaires. Personnel may update or correct their information as detailed in our internal company employment policies.\n' +
  'Customers have the right to request the restriction of certain uses and disclosures of personally identifiable information as\n' +
  'follows. You can contact us in order to (1) update or correct your personally identifiable information, (2) change your preferences\n' +
  'with respect to communications and other information you receive from us, or (3) delete the personally identifiable information\n' +
  'maintained about you on our systems (subject to the following paragraph), by cancelling your account. Such updates,\n' +
  'corrections, changes and deletions will have no effect on other information that we maintain, or information that we have\n' +
  'provided to third parties in accordance with this Privacy Policy prior to such update, correction, change or deletion. To protect\n' +
  'your privacy and security, we may take reasonable steps (such as requesting a unique password) to verify your identity before\n' +
  'granting you profile access or making corrections. You are responsible for maintaining the secrecy of your unique password and\n' +
  'account information at all times.\n' +
  'You should be aware that it is not technologically possible to remove each and every record of the information you have\n' +
  'provided to us from our system. The need to back up our systems to protect information from inadvertent loss means that a\n' +
  'copy of your information may exist in a non-erasable form that will be difficult or impossible for us to locate. Promptly after\n' +
  'receiving your request, all personal information stored in databases we actively use, and other readily searchable media will be\n' +
  'updated, corrected, changed or deleted, as appropriate, as soon as and to the extent reasonably and technically practicable.\n' +
  'If you are an end user and wish to update, delete, or receive any information we have about you, you may do so by contacting\n' +
  'the organization of which you are a customer.\n' +
  '<h3>Personnel</h3>\n' +
  '<p>If you are a VIP Billionaires worker or applicant, we collect information you voluntarily provide to us. We use the information\n' +
  'collected for Human Resources purposes in order to administer benefits to workers and screen applicants.\n' +
  'You may contact us in order to (1) update or correct your information, (2) change your preferences with respect to\n' +
  'communications and other information you receive from us, or (3) receive a record of the information we have relating to you.\n' +
  'Such updates, corrections, changes and deletions will have no effect on other information that we maintain, or information that\n' +
  'we have provided to third parties in accordance with this Privacy Policy prior to such update, correction, change or deletion.\n' +
  '<h3>Sale of Business</h3>\n' +
  '<p>We reserve the right to transfer information to a third party in the event of a sale, merger or other transfer of all or substantially\n' +
  'all of the assets of VIP Billionaires or any of its Corporate Affiliates (as defined herein), or that portion of VIP Billionaires or any\n' +
  'of its Corporate Affiliates to which the Service relates, or in the event that we discontinue our business or file a petition or have\n' +
  'filed against us a petition in bankruptcy, reorganization or similar proceeding, provided that the third party agrees to adhere to\n' +
  'the terms of this Privacy Policy.\n' +
  '<h3>Affiliates</h3>\n' +
  '<p>We may disclose information (including personal information) about you to our Corporate Affiliates. For purposes of this Privacy\n' +
  'Policy, "Corporate Affiliate" means any person or entity which directly or indirectly controls, is controlled by or is under common\n' +
  'control with VIP Billionaires, whether by ownership or otherwise. Any information relating to you that we provide to our\n' +
  'Corporate Affiliates will be treated by those Corporate Affiliates in accordance with the terms of this Privacy Policy.\n' +
  '<h3>Governing Law</h3>\n' +
  '<p>This Privacy Policy is governed by the laws of Japan without regard to its conflict of laws provision. You consent to the exclusive\n' +
  'jurisdiction of the courts in connection with any action or dispute arising between the parties under or in connection with this\n' +
  'Privacy Policy except for those individuals who may have rights to make claims under Privacy Shield, or the Swiss-US\n' +
  'framework.</p>\n' +
  '<p>The laws of Japan, excluding its conflicts of law rules, shall govern this Agreement and your use of the app. Your use of the app\n' +
  'may also be subject to other local, state, national, or international laws.</p>\n' +
  '<p>By using VIP Billionaires or contacting us directly, you signify your acceptance of this Privacy Policy. If you do not agree to this\n' +
  'Privacy Policy, you should not engage with our website, or use our services. Continued use of the website, direct engagement\n' +
  'with us, or following the posting of changes to this Privacy Policy that do not significantly affect the use or disclosure of your\n' +
  'personal information will mean that you accept those changes.</p>\n' +
  '<h3>Your Consent</h3>\n' +
  '<p>We\'ve updated our Privacy Policy to provide you with complete transparency into what is being set when you visit our site and\n' +
  'how it\'s being used. By using our app, registering an account, or making a purchase, you hereby consent to our Privacy Policy\n' +
  'and agree to its terms.\n' +
  'Links to Other Websites\n' +
  'This Privacy Policy applies only to the Services. The Services may contain links to other websites not operated or controlled by\n' +
  'VIP Billionaires. We are not responsible for the content, accuracy or opinions expressed in such websites, and such websites\n' +
  'are not investigated, monitored or checked for accuracy or completeness by us. Please remember that when you use a link to\n' +
  'go from the Services to another website, our Privacy Policy is no longer in effect. Your browsing and interaction on any other\n' +
  'website, including those that have a link on our platform, is subject to that website’s own rules and policies. Such third parties\n' +
  'may use their own cookies or other methods to collect information about you.\n' +
  '<h3>Cookies</h3>\n' +
  '<p>VIP Billionaires uses "Cookies" to identify the areas of our website that you have visited. A Cookie is a small piece of data\n' +
  'stored on your computer or mobile device by your web browser. We use Cookies to enhance the performance and functionality\n' +
  'of our app but are non-essential to their use. However, without these cookies, certain functionality like videos may become\n' +
  'unavailable or you would be required to enter your login details every time you visit the app as we would not be able to\n' +
  'remember that you had logged in previously. Most web browsers can be set to disable the use of Cookies. However, if you\n' +
  'disable Cookies, you may not be able to access functionality on our website correctly or at all. We never place Personally\n' +
  'Identifiable Information in Cookies.\n' +
  '<h3>Blocking and disabling cookies and similar technologies</h3>\n' +
  '<p>Wherever you\'re located you may also set your browser to block cookies and similar technologies, but this action may block our\n' +
  'essential cookies and prevent our website from functioning properly, and you may not be able to fully utilize all of its features\n' +
  'and services. You should also be aware that you may also lose some saved information (e.g. saved login details, site\n' +
  'preferences) if you block cookies on your browser. Different browsers make different controls available to you. Disabling a\n' +
  'cookie or category of cookie does not delete the cookie from your browser, you will need to do this yourself from within your\n' +
  'browser, you should visit your browser\'s help menu for more information.\n' +
  '<h3>Remarketing Services</h3>\n' +
  '<p>We use remarketing services. What Is Remarketing? In digital marketing, remarketing (or retargeting) is the practice of serving\n' +
  'ads across the internet to people who have already visited your website. It allows your company to seem like they\'re “following”\n' +
  'people around the internet by serving ads on the websites and platforms they use most.\n' +
  'Payment Details\n' +
  'In respect to any credit card or other payment processing details you have provided us, we commit that this confidential\n' +
  'information will be stored in the most secure manner possible.\n' +
  '<h3>Kids\' Privacy</h3>\n' +
  '<p>We do not address anyone under the age of 13. We do not knowingly collect personally identifiable information from anyone\n' +
  'under the age of 13. If You are a parent or guardian and You are aware that Your child has provided Us with Personal Data,\n' +
  'please contact Us. If We become aware that We have collected Personal Data from anyone under the age of 13 without\n' +
  'verification of parental consent, We take steps to remove that information from Our servers.\n' +
  '<h3>Changes To Our Privacy Policy</h3>\n' +
  '<p>We may change our Service and policies, and we may need to make changes to this Privacy Policy so that they accurately\n' +
  'reflect our Service and policies. Unless otherwise required by law, we will notify you (for example, through our Service) before\n' +
  'we make changes to this Privacy Policy and give you an opportunity to review them before they go into effect. Then, if you\n' +
  'continue to use the Service, you will be bound by the updated Privacy Policy. If you do not want to agree to this or any updated\n' +
  'Privacy Policy, you can delete your account.\n' +
  '<h3>Third-Party Services</h3>\n' +
  '<p>We may display, include or make available third-party content (including data, information, applications and other products\n' +
  'services) or provide links to third-party websites or services ("Third- Party Services").</p>\n' +
  '<p>You acknowledge and agree that VIP Billionaires shall not be responsible for any Third-Party Services, including their accuracy,\n' +
  'completeness, timeliness, validity, copyright compliance, legality, decency, quality or any other aspect thereof. VIP Billionaires\n' +
  'does not assume and shall not have any liability or responsibility to you or any other person or entity for any Third-Party\n' +
  'Services.</p>\n' +
  '<p>Third-Party Services and links thereto are provided solely as a convenience to you and you access and use them entirely at\n' +
  'your own risk and subject to such third parties\' terms and conditions.\n' +
  '<h3>Facebook Pixel</h3>\n' +
  '<p>Facebook pixel is an analytics tool that allows you to measure the effectiveness of your advertising by understanding the actions\n' +
  'people take on your website. You can use the pixel to: Make sure your ads are shown to the right people. Facebook pixel may\n' +
  'collect information from your device when you use the service. Facebook pixel collects information that is held in accordance\n' +
  'with its Privacy Policy\n' +
  '<h3>Information about General Data Protection Regulation (GDPR)</h3>\n' +
  '<p>We may be collecting and using information from you if you are from the European Economic Area (EEA), and in this section of\n' +
  'our Privacy Policy we are going to explain exactly how and why is this data collected, and how we maintain this data under\n' +
  'protection from being replicated or used in the wrong way.\n' +
  'What is GDPR?\n' +
  'GDPR is an EU-wide privacy and data protection law that regulates how EU residents\' data is protected by companies and\n' +
  'enhances the control the EU residents have, over their personal data.\n' +
  'The GDPR is relevant to any globally operating company and not just the EU-based businesses and EU residents. Our\n' +
  'customers’ data is important irrespective of where they are located, which is why we have implemented GDPR controls as our\n' +
  'baseline standard for all our operations worldwide.\n' +
  '<h3>What is personal data?</h3>\n' +
  '<p>Any data that relates to an identifiable or identified individual. GDPR covers a broad spectrum of information that could be used\n' +
  'on its own, or in combination with other pieces of information, to identify a person. Personal data extends beyond a person’s\n' +
  'name or email address. Some examples include financial information, political opinions, genetic data, biometric data, IP\n' +
  'addresses, physical address, sexual orientation, and ethnicity.</p>\n' +
  '<p>The Data Protection Principles include requirements such as:</p>\n' +
  '<p>● Personal data collected must be processed in a fair, legal, and transparent way and should only be used in a way that a\n' +
  'person would reasonably expect.<br/>\n' +
  '● Personal data should only be collected to fulfil a specific purpose and it should only be used for that purpose.\n' +
  'Organizations must specify why they need the personal data when they collect it.<br/>\n' +
  '● Personal data should be held no longer than necessary to fulfil its purpose.<br/>\n' +
  '● People covered by the GDPR have the right to access their own personal data. They can also request a copy of their data,\n' +
  'and that their data be updated, deleted, restricted, or moved to another organization.</p>\n' +
  '<h3>Why is GDPR important?</h3>\n' +
  '<p>GDPR adds some new requirements regarding how companies should protect individuals\' personal data that they collect and\n' +
  'process. It also raises the stakes for compliance by increasing enforcement and imposing greater fines for breach. Beyond\n' +
  'these facts it\'s simply the right thing to do. At VIP Billionaires we strongly believe that your data privacy is very important and we\n' +
  'already have solid security and privacy practices in place that go beyond the requirements of this new regulation.\n' +
  '<h3>Individual Data Subject\'s Rights - Data Access, Portability and Deletion</h3>\n' +
  '<p>We are committed to helping our customers meet the data subject rights requirements of GDPR. VIP Billionaires processes or\n' +
  'stores all personal data in fully vetted, DPA compliant vendors. We do store all conversation and personal data for up to 6 years\n' +
  'unless your account is deleted. In which case, we dispose of all data in accordance with our Terms of Service and Privacy\n' +
  'Policy, but we will not hold it longer than 60 days.\n' +
  'We are aware that if you are working with EU customers, you need to be able to provide them with the ability to access, update,\n' +
  'retrieve and remove personal data. We got you! We\'ve been set up as self service from the start and have always given you\n' +
  'access to your data and your customers data. Our customer support team is here for you to answer any questions you might\n' +
  'have about working with the API.\n' +
  '<h3>California Residents</h3>\n' +
  '<p>The California Consumer Privacy Act (CCPA) requires us to disclose categories of Personal Information we collect and how we\n' +
  'use it, the categories of sources from whom we collect Personal Information, and the third parties with whom we share it, which\n' +
  'we have explained above.</p>\n' +
  '<p>We are also required to communicate information about rights California residents have under California law. You may exercise\n' +
  'the following rights:</p>\n' +
  '<p>● Right to Know and Access. You may submit a verifiable request for information regarding the: (1) categories of Personal\n' +
  'Information we collect, use, or share; (2) purposes for which categories of Personal Information are collected or used by\n' +
  'us; (3) categories of sources from which we collect Personal Information; and (4) specific pieces of Personal Information\n' +
  'we have collected about you.<br/>\n' +
  '● Right to Equal Service. We will not discriminate against you if you exercise your privacy rights.<br/>\n' +
  '● Right to Delete. You may submit a verifiable request to close your account and we will delete Personal Information about\n' +
  'you that we have collected.<br/>\n' +
  '● Request that a business that sells a consumer\'s personal data, not sell the consumer\'s personal data.\n' +
  'If you make a request, we have one month to respond to you. If you would like to exercise any of these rights, please contact\n' +
  'us.</p>\n' +
  '<p>We do not sell the Personal Information of our users.</p>\n' +
  '<p>For more information about these rights, please contact us.</p>\n' +
  '<h3>California Online Privacy Protection Act (CalOPPA)</h3>\n' +
  '<p>CalOPPA requires us to disclose categories of Personal Information we collect and how we use it, the categories of sources\n' +
  'from whom we collect Personal Information, and the third parties with whom we share it, which we have explained above.\n' +
  'CalOPPA users have the following rights:</p>\n' +
  '<p>● Right to Know and Access. You may submit a verifiable request for information regarding the: (1) categories of Personal\n' +
  'Information we collect, use, or share; (2) purposes for which categories of Personal Information are collected or used by\n' +
  'us; (3) categories of sources from which we collect Personal Information; and (4) specific pieces of Personal Information\n' +
  'we have collected about you.<br/>\n' +
  '● Right to Equal Service. We will not discriminate against you if you exercise your privacy rights.<br/>\n' +
  '● Right to Delete. You may submit a verifiable request to close your account and we will delete Personal Information about\n' +
  'you that we have collected.<br/>\n' +
  '● Right to request that a business that sells a consumer\'s personal data, not sell the consumer\'s personal data.<br/>\n' +
  'If you make a request, we have one month to respond to you. If you would like to exercise any of these rights, please contact\n' +
  'us.</p>\n' +
  'We do not sell the Personal Information of our users.\n' +
  'For more information about these rights, please contact us.\n' +
  '<h3>Contact Us</h3>\n' +
  '<p>Don\'t hesitate to contact us if you have any questions.</p>\n' +
  '<p>● Via Email: info@zedinternational.net</p>';

export const CONTENT_PRIVACY_POLICY_JP =
  '<h1>ご利用規約</h1>\n' +
  '<p>更新日：2021-09-27</p>\n' +
  '\n' +
  '<h3>一般規約</h3>\n' +
  '<p>VIP Billionaires にアクセスして注文することで、以下の利用規約に含まれるサービス条件に同意し、拘束されることを確認します。<br/>\n' +
  'この規約は、本ウェブサイト全体、電子メール、その他の種類のものに適用されます。これらの条件は、本ウェブサイト全体および利用者とVIP Billionaires との間の電子メールやその他の手段による通信全てに適用されます。<br/>\n' +
  'いかなる状況においても、VIP Billionaires チームは、直接的、間接的、特別、偶発的または派生的な損害、データや利益の損失を含む、またはこれに限定されないいかなる損害に対しても、たとえ、VIP Billionaires チームまたはその公認代理人がそのような損害の可能性について知らされていたとしても責任を負いません。もしお客様が本サイトの資料資材を使用した結果、機器やデータの整備、修理、修正が必要になった場合、その費用はすべてお客様の負担となります。<br/>\n' +
  'VIP Billionaires は、お客様が当サイトの資料資材を使用する過程で発生したいかなる結果に対しても責任を負いません。当社は価格を変更したり、資料資材の使用ポリシーを変更する権利を有します。</p>\n' +
  '\n' +
  '<h3>ライセンス</h3>\n' +
  'VIP Billionaires は、本規約の条件に従って本アプリをダウンロード、インストール、使用するための、取消可能、非独占的、譲渡不能な限定的ライセンスをお客様に付与します。<br/>\n' +
  '本規約は、お客様とVIP Billionaires のウェブサイトおよびVIP Billionaires のウェブサイトからアクセスできるサービス（本規約では「VIP Billionaires サービス」と総称します）の提供者であるVIP Billionaires （本規約では「VIP Billionaires 」、「弊社」、「私たち」といいます）との間の契約です。<br/>\n' +
  'お客様は本利用規約に拘束されることに同意します。<br/>本利用規約に同意できない場合はVIP Billionaires サービスはご利用いただけません。本規約において「あなた」とは、個人としてのあなたと、あなたが代表する団体の両方を指します。<br/>\n' +
  'お客様が本規約のいずれかに違反した場合、当社は予告なしにお客様のアカウントを解約したり、お客様のアカウントへのアクセスを遮断する権利を有します。<br/>\n' +
  '\n' +
  '<h3>定義と主要な用語</h3>\n' +
  '<p>本利用規約では、できるかぎり説明を明解にするために、これらの用語が使用される際はつねに、厳密に以下の通りの定義によるものとします。</p>\n' +
  '<p>・ Cookie: Webサイトによって生成され、お客様のWebブラウザによって保存される少量のデータ。これは、お客様のブラウザを識別し分析するために使用されます。また、お客様の言語設定やログイン情報など、お客様に関する情報を記憶します。</p>\n' +
  '<p>・ 会社：本ポリシーで「会社」、「私たち」、「私たちの」と記載されている場合、それは日本の名古屋にあるゼンエルディーンインターナショナル株式会社を指し、本規約の下でお客様の情報に責任を有します。</p>\n' +
  '<p>・ 国：VIP Billionaires または VIP Billionaires の所有者/創設者が拠点を置く国、この場合は日本を指します。</p>\n' +
  '<p>・ デバイス：電話、タブレット、コンピューターなどのインターネットに接続するデバイスであり、VIP Billionaires にアクセスし、サービスを利用するために使用されるあらゆる機器を指します。</p>\n' +
  '<p>・ サービス：関連する条件（ある場合）や本プラットフォームに記載されている、VIP Billionaires が提供するサービスを指します。</p>\n' +
  '<p>・ 第三者のサービス：広告主、コンテストのスポンサー、プロモーションやマーケティングのパートナー、その他弊社のコンテンツを提供したり、お客様が興味を持ちそうな製品やサービスを提供する人を指します。</p>\n' +
  '<p>・ ウェブサイト：VIP Billionaires のサイトを指し、[vipbillionaires.com] のURLからアクセスできます。</p>\n' +
  '<p>・ あなた、およびお客様：本サービスを利用するために VIP Billionaires に登録された個人または法人を指します。</p>\n' +
  '\n' +
  '<h3>制限事項</h3>\n' +
  '<p>お客様は、以下の行為を行わないことに同意し、他者に許可しないものとします。<br/>\n' +
  '・ 本アプリのライセンス販売、レンタル、リース、割り当て、配布、送信、ホスティング、アウトソース、開示、その他商業的に利用すること、または第三者が本プラットフォームを利用可能にすること。<br/>\n' +
  '・ 本アプリのいかなる部分も修正、二次的著作物の作成、逆アセンブル、解読、逆コンパイル、リバースエンジニアリングすること。<br/>\n' +
  '・ VIP Billionaires またはその関連会社、パートナー、サプライヤー、本アプリのライセンサーの所有権表示（著作権や商標の表示を含む）を削除、変更、不明瞭にすること。</p>\n' +
  '\n' +
  '<h3>返品・返金について</h3>\n' +
  '<p>VIP Billionaires でお買い物をしていただきありがとうございます。<br/>私たちは、お客様が私たちの作ったものを気に入ってご購入くださることに感謝しています。<br/>また、私たちは、お客様が私たちの製品を探索し、評価し、購入する際に、満足のいく体験をしていただきたけるよう努めています。<br/>\n' +
  '他のショッピングと同様に、VIP Billionaires での取引には利用規約があります。<br/>他のショッピングと同様に、VIP Billionaires での取引には利用規約があります。ここでは、弁護士が許す限り ここでは、弁護士が許す限り簡潔に説明します。<br/>覚えておいていただきたいのは、VIP Billionaires で注文または購入することで、あなたはこの規約とVIP Billionaires の「プライバシーポリシー」に同意したことになるといくことです。<br/>\n' +
  '何らかの理由で当社が提供する商品やサービスに完全に満足できない場合は、遠慮なく当社にご連絡ください。<br/>\n' +
  '私たちの製品についての問題点をお話し合いさせていただきたく存じます。</p>\n' +
  '\n' +
  '<h3>ご提案</h3>\n' +
  '<p>本アプリに関してお客様からVIP Billionaires に提供されたフィードバック、コメント、アイデア、改善、提案（以下、総称して「提案」といいます）は、VIP Billionaires の単独かつ排他的な財産となります。<br/>\n' +
  'VIP Billionaires はご提案を、目的や方法を問わず、また、お客様への謝礼および承認を必要とせず、自由に使用、コピー、修正、公開、再配布することができます。<br/>\n' +
  'お客様の同意について\n' +
  'お客様が当サイトにアクセスした際に何が設定され、どのように使用されているかについて完全な透明性を提供するために、当社の利用規約を更新しました。<br/>アプリの使用、アカウントの登録、または購入を行うことで、お客様は当社の利用規約に同意したことになります。</p>\n' +
  '\n' +
  '<h3>他のウェブサイトへのリンク</h3>\n' +
  '<p>本規約は本サービスにのみ適用されます。本サービスにはVIP Billionaires が運営または管理していない他のウェブサイトへのリンクが含まれることもあります。<br/>弊社はそのようなウェブサイトで提示されているコンテンツ、正確性、意見について責任を負いません。そのようなウェブサイトは、弊社が調査、監視、正確性や完全性をチェックしたものではありません。本サービスからリンクを使って別のウェブサイトに移動した場合、弊社の利用規約の適用外となりますのでご注意ください。<br/>他のウェブサイト（当社のプラットフォームからリンクがあるものを含む）でのお客様の閲覧および活動は、そのウェブサイトの規則および方針に従うものとします。<br/>そのような第三者は、独自のクッキーまたはその他の方法を使用して、お客様に関する情報を収集することがあります。</p>\n' +
  '\n' +
  '<h3>クッキー</h3>\n' +
  '<p>VIP Billionaires は、お客様がご覧になった弊社のウェブサイトの領域を識別するために「クッキー」を使用しています。<br/>クッキーとは ウェブ・ブラウザによって利用者のコンピュータやモバイル・デバイスに保存される小さなデータです。当社は、当社アプリのパフォーマンスと機能性を向上させるためにクッキーを使用していますが、その使用は必須ではありません。クッキーとは、ウェブ・ブラウザによって利用者のコンピュータやモバイル・デバイスに保存される小さなデータです。当社は、当社のアプリのパフォーマンスと機能を向上させるためにクッキーを使用していますが、その使用は必須ではありません。<br/>ただし、これらのCookieがないと、ビデオなどの特定の機能が利用できなくなったり、お客様が以前にログインしたことを覚えていないために、アプリにアクセスするたびにログイン情報の入力が必要になったりすることがあります。ほとんどのウェブブラウザは、Cookieの使用を無効にするように設定できます。しかし、お客様がCookieを無効にした場合、当社ウェブサイトの機能に正しくまたは全くアクセスできなくなる可能性があります。<br/>当社は、個人を特定できるような情報をCookieに入れることはありません。</p>\n' +
  '\n' +
  '<h3>ご利用規約の変更</h3>\n' +
  '<p>お客様は、VIP Billionaires の独自の判断により、お客様への事前の通知なしに、本サービス（または本サービス内の機能）のお客様または一般利用者への提供を（永久的または一時的に）停止することができることを理解し、同意するものとします。お客様は、いつでも本サービスの利用を中止することができます。本サービスの使用を中止する場合、特にVIP Billionaires に通知する必要はありません。<br/>お客様は、VIP Billionaires がお客様のアカウントへのアクセスを無効にした場合、お客様は以下のことができなくなることを理解し、同意するものとします。<br/>当社が利用規約の変更を決定した場合、当社はその変更をこのページに掲載し、下記の利用規約の変更日を更新します。</p>\n' +
  '\n' +
  '<h3>当社のアプリの修正</h3>\n' +
  '<p>VIP Billionaires は、予告の有無にかかわらず、お客様に責任を負うことなく、一時的または永続的に本アプリまたは本アプリに接続しているサービスを変更、中断、中止する権利を有します。</p>\n' +
  '\n' +
  '<h3>当社アプリの更新</h3>\n' +
  '<p>VIP Billionaires は、随時、本アプリの機能を強化または改善することがあります。<br/>\n' +
  'これにはパッチ、バグフィックス、アップデート、アップグレード、その他の変更が含まれます（以下「アップデート」といいます）。<br/>\n' +
  'アップデートは本アプリの特定の機能を変更または削除する場合があります。<br/>お客様は、VIP Billionaires が(i)アップデートを提供する義務を負わないこと、または(ii)本アプリの特定の機能をお客様に提供し続ける、もしくは有効にする義務を負わないことに同意するものとします。<br/>\n' +
  'さらにお客様は、すべてのアップデートが(i)本アプリの不可欠な部分を構成するとみなされ、(ii)本契約の条件に従うことに同意するものとします。</p>\n' +
  '\n' +
  '<h3>第三者のサービス</h3>\n' +
  '<p>本サイトでは、第三者のコンテンツ（データ、情報、アプリケーション、その他の製品サービスを含む）を表示、掲載、利用可能にしたり、第三者のウェブサイトやサービスへのリンクを提供することがあります（以下「第三者サービス」）。<br/>\n' +
  'お客様は、第三者サービスの正確性、完全性、適時性、有効性、著作権の遵守、合法性、良識、品質またはその他の側面を含めて、VIP Billionairesが第三者サービスに対して責任を負わないことを認識し、同意するものとします。VIP Billionaires は、第三者のサービスについて、お客様またはその他の人や団体に対して、いかなる責任も負わないものとします。<br/>\n' +
  '第三者サービスおよびそのリンクは、利用者の利便性のためにのみ提供されており、利用者は完全に自分の責任で当該第三者の利用規約に従ってアクセスし、使用するものとします。</p>\n' +
  '\n' +
  '<h3>契約期間と解除</h3>\n' +
  '<p>本契約は、お客様またはVIP Billionaires が終了するまで有効とします。<br/>\n' +
  'VIP Billionaires は、独自の判断により、いつでも、いかなる理由でも、事前の通知の有無にかかわらず、本契約を中断または終了することができます。<br/>\n' +
  '本契約は、お客様が本契約のいずれかの条項に従わなかった場合、VIP Billionaires からの事前の通知なしに直ちに終了します。また、お客様は、本アプリおよびそのすべてのコピーをお客様のコンピュータから削除することにより、本契約を終了することができます。\n' +
  '本契約の終了に伴い、お客様は本アプリの使用をすべて中止し、本アプリのすべてのコピーをお客様のコンピュータから削除するものとします。<br/>\n' +
  '本契約の終了は、（本契約の期間中に）お客様が本契約に基づく義務のいずれかに違反した場合、法律上または資産に関するVIP Billionaires の権利または救済手段を制限するものではありません。<br/>\n' +
  '著作権侵害の通知\n' +
  'お客様が著作権所有者またはその代理人であり、当社のアプリ上の素材がお客様の著作権の侵害を構成していると思われる場合は 以下の情報を添えて、当社にご連絡ください。<br/>本契約の終了に伴い、お客様は本アプリの使用をすべて中止し、本アプリのすべてのコピーをお客様のコンピュータから削除するものとします。<br/>\n' +
  '本契約の終了は、以下の違反があった場合の法律上または衡平法上の VIP Billionaires の権利または救済手段を制限するものではありません。<br/>本契約の終了は、（本契約の期間中に）お客様が本契約に基づく義務のいずれかに違反した場合、法律上または衡平法上のVIP Billionairesの権利または救済手段を制限するものではありません。</p>\n' +
  '\n' +
  '<h3>著作権侵害の通知</h3>\n' +
  '<p>お客様が著作権所有者またはその代理人であり、当社のアプリ上の素材がお客様の著作権の侵害を構成していると思われる場合は 以下の情報を添えて、当社にご連絡ください。<br/>(a) 著作権所有者またはその代理人の物理的または電子的な署名(a) 著作権所有者またはその代理人の物理的または電子的な署名 (b) 侵害していると主張する素材の識別情報 (c) お客様の連絡先情報（住所、電話番号、メールアドレスなど）。<br/>(c) 住所、電話番号、電子メールなどの連絡先情報 (d) 素材の使用が許可されていないという誠実な信念を持っていることを示す、お客様による陳述書 (e) 通知に記載された情報が正確であり、偽証罪に基づき、あなたが著作権者を代表して行動することを許可されているという声明。\n' +
  '補償\n' +
  'お客様は、VIP Billionaires およびその親会社、子会社、関連会社、役員、従業員、代理人、パートナーおよびライセンサー（存在する場合）を補償し、保持することに同意するものとします。<br/>以下の事項に起因する請求または要求（合理的な弁護士費用を含む）から、VIP Billionaires およびその親会社、子会社、関連会社、役員、従業員、代理人、パートナー、ライセンサー（もしあれば）を免責することに同意するものとします。<br/>(a)  本アプリの使用、(b) 本契約または法令の違反、または(c) 第三者の権利の侵害。</p>\n' +
  '\n' +
  '<h3>保証なし</h3>\n' +
  '<p>本アプリは、「現状有姿」で提供されており、いかなる種類の保証もなく、すべての欠点および欠陥がある状態で提供されています。適用法で認められている最大限の範囲で、VIP Billionaires は、自身、その関連会社、そのライセンサー、サービスプロバイダーを代表して、明示的、黙示的、法定、その他を問わず、本アプリに関するすべての保証を明示的に放棄します。これには、商品性、特定目的への適合性、権原、権利を侵害していないことに関するすべての黙示的な保証、取引の過程、履行の過程、使用、取引慣行から生じる保証が含まれます。<br/>\n' +
  '上記に限定されず、VIP Billionaires は、本アプリがお客様の要件を満たし、意図した結果を達成し、他のソフトウェア、システム、サービスと互換性があり、機能し、中断することなく動作し、性能や信頼性の基準を満たし、エラーがなく、エラーや欠陥が修正されることを保証または約束せず、いかなる種類の表明も行いません。また、エラーや欠陥が修正されることもありません。<br/>\n' +
  '上記を制限することなく、VIP Billionaires および VIP Billionaires の提供者は、明示的または黙示的を問わず、いかなる種類の表明または保証も行いません。<br/>(i)本アプリの動作や利用可能性、またはそこに含まれる情報、コンテンツ、素材や製品に関すること、(ii)本アプリが中断されない、またはエラーがないこと、(iii)本アプリを通じて提供される情報やコンテンツの正確性、信頼性、または通貨に関すること、または(iv)本アプリ、そのサーバー、コンテンツ、またはVIP BillionairesからもしくはVIP Billionairesを代表して送信される電子メールに、ウイルス、スクリプト、トロイの木馬、ワーム、マルウェア、タイムボム、その他の有害な要素がないこと。<br/>\n' +
  '管轄区域によっては、黙示的な保証の除外や制限、消費者の適用される法定権利の制限を認めていないため、上記の除外や制限の一部または全部がお客様に適用されない場合があります。</p>\n' +
  '\n' +
  '<h3>責任の制限</h3>\n' +
  '<p>お客様が被る可能性のある損害にかかわらず、本契約のいずれかの条項に基づく VIP Billionaires およびその供給者の全責任、および上記のすべてに対するお客様の唯一の救済手段は、本アプリに対してお客様が実際に支払った金額に限定されます。<br/>\n' +
  '適用法で認められている最大限の範囲で、いかなる場合も、VIP Billionaires Billionaires およびその供給者は、特別、偶発的、間接的、または結果的な損害（利益の損失、データやその他の情報の損失、事業の中断、個人的な傷害、プライバシーの喪失などを含むがこれらに限定されない）について責任を負いません。本アプリの使用または使用できないこと、第三者のソフトウェアおよび／または本アプリとともに使用される第三者のハードウェア、あるいは本契約のいずれかの規定に関連して生じるプライバシーの喪失）については、たとえVIP Billionaires またはサプライヤーがそのような損害の可能性を知らされていたとしても、また、救済策が本質的な目的を果たせなかったとしても、一切の責任を負いません。<br/>\n' +
  '州や司法権によっては、偶発的または結果的な損害の除外または制限を認めていないため、上記の制限または除外が 上記の制限はお客様に適用されない場合があります。</p>\n' +
  '\n' +
  '<h3>分離可能性</h3>\n' +
  '<p>本契約のいずれかの条項が執行不能または無効であると判断された場合、当該条項は適用法の下で可能な限りその目的を達成するように変更および解釈され、残りの条項は完全な効力を持ち続けるものとします。<br/>\n' +
  '本規約は、プライバシーポリシーおよびVIP Billionaires が本サービス上で公表するその他の法的通知とともに、本サービスに関するお客様とVIP Billionaires との間の完全な合意を構成するものとします。<br/>本規約のいずれかの条項が管轄裁判所によって無効と判断された場合でも、当該条項の無効性は本規約の残りの条項の有効性に影響を与えず、それらは完全に効力を有するものとします。<br/>本契約のいずれかの条項を放棄しても、その条項またはその他の条項をさらにまたは継続して放棄したとはみなされません。<br/>また、VIP Billionaires が本契約のいずれかの権利または条項を主張しなかったとしても、そのような権利または条項を放棄したとはみなされません。<br/>お客様と VIP Billionaires は、本サービスに起因または関連するいかなる訴因も、訴因発生後 1 年以内に開始しなければならないことに同意します。<br/>そうでない場合は、かかる訴訟原因は永久に禁止されます。</p>\n' +
  '\n' +
  '<h3>権利放棄</h3>\n' +
  '<p>本契約に規定されている場合を除き、本契約に基づく権利の行使または義務の履行を要求しなかった場合でも、その後いつでも当該権利を行使したり、当該履行を要求したりする当事者の能力に影響を与えるものではなく、また、ある違反を放棄した場合でも、その後の違反を放棄したことにはならないものとします。<br/>\n' +
  '本契約に基づくいかなる権利または権限についても、いずれかの当事者が行使を怠ったり、行使を遅らせたりしても、その権利または権限を放棄したことにはなりません。<br/>また、本契約に基づく権利または権限を単独または部分的に行使した場合、当該権利または本契約で認められているその他の権利のさらなる行使を妨げるものではありません。本契約と、適用される購入条件またはその他の条件との間に矛盾がある場合は、本契約の条件が優先されます。</p>\n' +
  '\n' +
  '<h3>本規約の修正</h3>\n' +
  '<p>VIP Billionaires は、独自の判断により、いつでも本規約を修正または変更する権利を有します。修正が重要である場合 は、新しい条件が有効になる前に、少なくとも30日前に通知します。何が重要な変更であるかは 当社の独自の裁量により決定されます。\n' +
  '改訂が有効になった後も、当社のアプリへのアクセスまたは使用を継続することにより、お客様は改訂された条件に拘束されることに同意するものとします。<br/>もしお客様が 新しい規約に同意しない場合は、もはやVIP Billionaires を使用する権限はありません。</p>\n' +
  '\n' +
  '<h3>完全合意</h3>\n' +
  '<p>本契約は、本アプリの使用に関するお客様とVIP Billionaires との間の完全な合意を構成するものであり、お客様とVIP Billionaires との間で交わされたすべての事前の書面または口頭による合意に優先します。お客様は、他の VIP Billionaires のサービスを使用または購入する際に適用される追加の規約の対象となる場合があり、VIP Billionaires はそのような使用または購入時にお客様に提供します。 </p>\n' +
  '\n' +
  '<h3>規約の更新</h3>\n' +
  '弊社は、弊社のサービスおよびポリシーを変更することがあり、弊社のサービスおよびポリシーを正確に反映するために、本規約の変更が必要になることがあります。<br/>法律で義務付けられていない限り、当社は、本規約を変更する前にお客様に通知し（例えば、当社のサービスを通じて）、本規約が発効する前にお客様に確認する機会を提供します。<br/>その後、お客様が本サービスの利用を継続する場合、お客様は更新された規約に拘束されることになります。本規約または更新された規約に同意したくない場合は、お客様のアカウントを削除することができます。</p>\n' +
  '\n' +
  '<h3>知的財産</h3>\n' +
  '<p>本アプリとそのコンテンツ全体、機能、特徴（すべての情報、ソフトウェア、テキスト、ディスプレイ、画像、ビデオ、オーディオ、およびそれらのデザイン、選択、配置を含みますが、これらに限定されません）は、VIP Billionaires 、そのライセンサー、またはそのような素材の他の提供者が所有しており、日本および国際的な著作権、商標、特許、企業秘密、その他の知的財産権や所有権に関する法律によって保護されています。これらの資料は、本利用規約で明示的に規定されている場合を除き、VIP Billionaires の書面による事前の許可なしに、全体または一部を問わず、いかなる方法でもコピー、修正、複製、ダウンロード、配布することはできません。<br/>\n' +
  '本規約で明示的に規定されている場合を除きます。<br/>本資料の無断使用は禁止されています。</p>\n' +
  '\n' +
  '<h3>仲裁に関する合意</h3>\n' +
  '<p>本条項は、お客様または VIP Billionaires の知的財産権の行使または有効性に関する、仲裁または衡平法上の救済の請求に関連する紛争を含まない場合を除き、あらゆる紛争に適用されます。<br/>紛争」とは、契約、保証、不法行為、法令、規制、条例、その他の法的または衡平法上の根拠に関わらず、本サービスまたは本契約に関するお客様とVIP Billionaires との間の紛争、訴訟、その他の論争を意味します。<br/>「紛争」は、法律に基づいて可能な限り広い意味で解釈されます。</p>\n' +
  '\n' +
  '<h3>争議の通知</h3>\n' +
  '<p>紛争が発生した場合、お客様またはVIP Billionaires は相手側に紛争通知を提出しなければなりません。<br/>\n' +
  '紛争通知は、通知する側の名前、住所、連絡先、紛争の原因となっている事実、要求されている救済策を記載した書面です。<br/>紛争通知は電子メールで info@zedinternational.net 宛てに送付してください。<br/>VIP Billionaires は争議通知をお客様の住所がわかっている場合はその住所に、それ以外の場合はお客様の電子メールアドレスに郵送します。<br/>お客様と VIP Billionaires は、紛争の通知が送付された日から 60 日以内に、非公式な交渉によって紛争の解決を図るものとします。<br/>60 日を経過した場合、お客様または VIP Billionaires は仲裁を開始することができます。</p>\n' +
  '\n' +
  '<h3>拘束力のある調停</h3>\n' +
  '<p>お客様とVIP Billionaires が非公式な交渉によって紛争を解決できない場合、紛争を解決するためのその他の努力は、本項に記載されている拘束力のある仲裁によってのみ行われます。<br/>利用者は、すべての紛争を裁判官または陪審員の前で法廷に提訴する（または当事者もしくは集団の一員として参加する）権利を放棄します。<br/>紛争は、米国仲裁協会の商事仲裁規則に基づく拘束力のある仲裁によって解決されるものとします。<br/>いずれの当事者も、仲裁が完了するまでの間、当事者の権利または財産を保護するために必要であれば、管轄権を有する裁判所に暫定的または仮の差止命令による救済を求めることができます。<br/>勝訴した当事者が負担したすべての法律上、会計上、およびその他の費用、手数料、および経費は、勝訴しなかった当事者が負担するものとします。</p>\n' +
  '\n' +
  '<h3>提出物およびプライバシー</h3>\n' +
  '<p>お客様がアイデア、創造的な提案、デザイン、写真、情報、広告、データまたは提案（新しいまたは改良された製品、サービス、機能、技術またはプロモーションのアイデアを含む）を提出または投稿した場合、お客様は、そのような提出物が自動的に非機密的かつ非独占的なものとして扱われ、お客様へのいかなる報酬または信用なしにVIP Billionaires の唯一の財産となることに明示的に同意するものとします。<br/>VIP Billionaires およびその関連会社は、そのような提出物や投稿に関していかなる義務も負わないものとし、そのような提出物や投稿に含まれるアイデアを、そのようなアイデアを使用した製品やサービスの開発、製造、マーケティングを含むがそれに限定されない、いかなる目的のためにいかなる媒体でも永続的に使用することができるものとします。</p>\n' +
  '\n' +
  '<h3>プロモーション</h3>\n' +
  '<p>VIP Billionaires は、随時、コンテスト、プロモーション、懸賞、その他の活動（以下「プロモーション」といいます）を行い、お客様にご自身に関する資料や情報の提出をお願いする場合があります。<br/>すべてのプロモーションは、年齢や地域の制限など、特定の資格要件を含む別の規則によって管理されていることにご注意ください。<br/>お客様は、ご自身が参加する資格があるかどうかを判断するために、すべてのプロモーションの規則を読む責任があります。<br/>お客様がプロモーションに参加する場合、お客様はすべてのプロモーション規則を遵守することに同意するものとします。<br/>\n' +
  '本サービス上または本サービスを通じての商品またはサービスの購入には、追加の条件が適用される場合がありますが、これらの条件は、この参照により本契約の一部となります。</p>\n' +
  '\n' +
  '<h3>誤字脱字について</h3>\n' +
  '<p>誤植により、製品やサービスが誤った価格で表示されていたり、誤った情報が記載されていたりした場合、当社は、誤った価格で表示された商品およびサービスの注文を拒否または取り消す権利を有するものとします。<br/>当社は、注文が確定し、お客様のクレジットカードに課金されたか否かにかかわらず、かかる注文を拒否または取り消す権利を有するものとします。<br/>もしお客様のクレジットカードが既に課金されており、注文がキャンセルされた場合、当社は、お客様のクレジットカード口座またはその他の支払口座に、請求額に相当する金額を直ちに返金するものとします。</p>\n' +
  '\n' +
  '<h3>その他</h3>\n' +
  '<p>理由の如何を問わず、管轄裁判所が本規約のいずれかの条項または一部が法的効力を持たないと判断した場合でも、本規約の残りの部分は完全に効力を有します。<br/>本規約のいかなる条項の放棄も、権限のあるVIP Billionaires の代表者が署名した書面によってのみ有効となります。VIP Billionaires は、利用者による違反または違反が予想される場合、差し止めによる救済またはその他の衡平法上の救済を受ける権利を有します（保証金を支払う義務はありません）。<br/>VIP Billionaires は、日本国内の事務所からVIP Billionaires のサービスを運営、管理します。<br/>本サービスは、その配布や使用が法律や規制に反する司法管轄区や国の個人や団体への配布や使用を意図したものではありません。<br/>したがって、他の場所からVIP Billionaires のサービスにアクセスすることを選択した方は、自らの意思でアクセスし、現地の法律が適用される場合は、その範囲内で現地の法律を遵守することに単独で責任を負います。<br/>本規約（VIP Billionaires のプライバシー ポリシーを含む）には、その主題に関するお客様と VIP Billionaires との間の完全な理解が含まれており、それ以前のすべての理解に取って代わるものであり、お客様が変更または修正することはできません。<br/>本契約で使用されているセクションの見出しは便宜上のものであり、法的な意味を持つものではありません。</p>\n' +
  '\n' +
  '<h3>免責事項</h3>\n' +
  '<p>VIP Billionaires は、いかなるコンテンツ、コード、その他の不正確な情報に対しても責任を負いません。<br/>\n' +
  'VIP Billionaires は保証を提供しません。<br/>\n' +
  'いかなる場合も、VIP Billionaires は、本サービスの使用または本サービスのコンテンツに関連して発生した、特別、直接的、間接的、結果的、または偶発的な損害、または契約、過失、その他の不法行為にかかわらず、いかなる損害に対しても責任を負いません。当社は、本サービス上のコンテンツについて、事前に通知することなく、いつでも追加、削除、変更を行うことができます。<br/>\n' +
  'VIP Billionaires のサービスおよびそのコンテンツは、「現状のまま」および「利用可能な状態で」提供され、明示または黙示を問わず、いかなる種類の保証も表明もありません。VIP Billionaires は第三者が提供するコンテンツの配信者であり、発行者ではありません。<br/>\n' +
  'VIP Billionaires のサービスを通じて提供される、または VIP Billionaires のサービスからアクセスできる情報、コンテンツ、サービスまたは商品の正確性、信頼性または通貨に関して、いかなる保証も表明も行いません。上記を制限することなく、VIP Billionaires は以下の事項について一切の保証および表明を行いません。VIP Billionaires のサービスに関連して送信されたコンテンツ、VIP Billionaires のサービスにリンクされているサイト、または、VIP Billionaires のサービスの一部として、またはVIP Billionaires のサービスに関連して提供される製品についても、一切の保証および表明を行いません。<br/>商品性、特定目的への適合性、第三者の権利を侵害していないことの保証と表明を否認します。VIP Billionaires またはその関連会社、従業員、役員、取締役、代理人などによる口頭でのアドバイスや書面での情報提供について保証はありません。<br/>価格や入手可能性の情報は、予告なく変更されることがあります。上記を制限することなく VIP Billionaires は、VIP Billionaires のサービスが中断されず、時宜にかなったものであり、誤りがないことを保証するものではありません。</p>\n' +
  '\n' +
  '<h3>お問い合わせ</h3>\n' +
  '<p>ご不明な点がございましたら、ご遠慮なくお問い合わせください。</p>\n' +
  '<p>電子メールで: info@zedinternational.net</p>\n';

export const CONTENT_TERMS_AND_CONDITIONS =
  '<h2>Terms & Conditions</h2>\n' +
  '<p>Updated at 2021-09-27</p>\n' +
  '<h3>General Terms</h3>\n' +
  '<p>By accessing and placing an order with VIP Billionaires, you confirm that you are in agreement with and bound by the terms of\n' +
  'service contained in the Terms & Conditions outlined below. These terms apply to the entire website and any email or other type\n' +
  'of communication between you and VIP Billionaires.</p>\n' +
  '<p>Under no circumstances shall VIP Billionaires team be liable for any direct, indirect, special, incidental or consequential\n' +
  'damages, including, but not limited to, loss of data or profit, arising out of the use, or the inability to use, the materials on this\n' +
  'site, even if VIP Billionaires team or an authorized representative has been advised of the possibility of such damages. If your\n' +
  'use of materials from this site results in the need for servicing, repair or correction of equipment or data, you assume any costs\n' +
  'thereof.</p>\n' +
  '<p>VIP Billionaires will not be responsible for any outcome that may occur during the course of usage of our resources. We reserve\n' +
  'the rights to change prices and revise the resources usage policy in any moment.</p>\n' +
  '<h3>License</h3>\n' +
  '<p>VIP Billionaires grants you a revocable, non-exclusive, non-transferable, limited license to download, install and use the app\n' +
  'strictly in accordance with the terms of this Agreement.</p>\n' +
  '<p>These Terms & Conditions are a contract between you and VIP Billionaires (referred to in these Terms & Conditions as "VIP\n' +
  'Billionaires", "us", "we" or "our"), the provider of the VIP Billionaires website and the services accessible from the VIP\n' +
  'Billionaires website (which are collectively referred to in these Terms & Conditions as the "VIP Billionaires Service").\n' +
  '<p>You are agreeing to be bound by these Terms & Conditions. If you do not agree to these Terms & Conditions, please do not use\n' +
  'the VIP Billionaires Service. In these Terms & Conditions, "you" refers both to you as an individual and to the entity you\n' +
  'represent. If you violate any of these Terms & Conditions, we reserve the right to cancel your account or block access to your\n' +
  'account without notice.</p>\n' +
  '<h3>Definitions and key terms</h3>\n' +
  '<p>To help explain things as clearly as possible in this Terms & Conditions, every time any of these terms are referenced, are\n' +
  'strictly defined as:</p>\n' +
  '<p>● Cookie: small amount of data generated by a website and saved by your web browser. It is used to identify your browser,\n' +
  'provide analytics, remember information about you such as your language preference or login information.<br/>\n' +
  '● Company: when this policy mentions “Company,” “we,” “us,” or “our,” it refers to Zein El Dine International Co., Ltd, Tokyo,\n' +
  'Japan that is responsible for your information under this Terms & Conditions.<br/>\n' +
  '● Country: where VIP Billionaires or the owners/founders of VIP Billionaires are based, in this case is Japan<br/>\n' +
  '● Device: any internet connected device such as a phone, tablet, computer or any other device that can be used to visit VIP\n' +
  'Billionaires and use the services.<br/>\n' +
  '● Service: refers to the service provided by VIP Billionaires as described in the relative terms (if available) and on this\n' +
  'platform.<br/>\n' +
  '● Third-party service: refers to advertisers, contest sponsors, promotional and marketing partners, and others who provide\n' +
  'our content or whose products or services we think may interest you.<br/>\n' +
  '● Website: VIP Billionaires."’s" site, which can be accessed via this URL:<br/>\n' +
  '● You: a person or entity that is registered with VIP Billionaires to use the Services.</p>\n' +
  '<h3>Restrictions</h3>\n' +
  '<p>You agree not to, and you will not permit others to:</p>\n' +
  '<p>● License, sell, rent, lease, assign, distribute, transmit, host, outsource, disclose or otherwise commercially exploit the app or\n' +
  'make the platform available to any third party.<br/>\n' +
  '● Modify, make derivative works of, disassemble, decrypt, reverse compile or reverse engineer any part of the app.<br/>\n' +
  '● Remove, alter or obscure any proprietary notice (including any notice of copyright or trademark) of VIP Billionaires or its\n' +
  'affiliates, partners, suppliers or the licensors of the app.</p>\n' +
  '<h3>Return and Refund Policy</h3>\n' +
  '<p>Thanks for shopping at VIP Billionaires. We appreciate the fact that you like to buy the stuff we build. We also want to make\n' +
  'sure you have a rewarding experience while you’re exploring, evaluating, and purchasing our products.</p>\n' +
  '<p>As with any shopping experience, there are terms and conditions that apply to transactions at VIP Billionaires. We’ll be as brief\n' +
  'as our attorneys will allow. The main thing to remember is that by placing an order or making a purchase at VIP Billionaires, you\n' +
  'agree to the terms along with VIP Billionaires."’s" Privacy Policy.</p>\n' +
  '<p>If, for any reason, You are not completely satisfied with any good or service that we provide, don\'t hesitate to contact us and we\n' +
  'will discuss any of the issues you are going through with our product.</p>\n' +
  '<h3>Your Suggestions</h3>\n' +
  '<p>Any feedback, comments, ideas, improvements or suggestions (collectively, "Suggestions") provided by you to VIP Billionaires\n' +
  'with respect to the app shall remain the sole and exclusive property of VIP Billionaires.</p>\n' +
  '<p>VIP Billionaires shall be free to use, copy, modify, publish, or redistribute the Suggestions for any purpose and in any way\n' +
  'without any credit or any compensation to you.</p>\n' +
  '<h3>Your Consent</h3>\n' +
  '<p>We\'ve updated our Terms & Conditions to provide you with complete transparency into what is being set when you visit our site\n' +
  'and how it\'s being used. By using our app, registering an account, or making a purchase, you hereby consent to our Terms &\n' +
  'Conditions.</p>\n' +
  '<h3>Links to Other Websites</h3>\n' +
  '<p>This Terms & Conditions applies only to the Services. The Services may contain links to other websites not operated or\n' +
  'controlled by VIP Billionaires. We are not responsible for the content, accuracy or opinions expressed in such websites, and\n' +
  'such websites are not investigated, monitored or checked for accuracy or completeness by us. Please remember that when you\n' +
  'use a link to go from the Services to another website, our Terms & Conditions are no longer in effect. Your browsing and\n' +
  'interaction on any other website, including those that have a link on our platform, is subject to that website’s own rules and\n' +
  'policies. Such third parties may use their own cookies or other methods to collect information about you.</p>\n' +
  '<h3>Cookies</h3>\n' +
  '<p>VIP Billionaires uses "Cookies" to identify the areas of our website that you have visited. A Cookie is a small piece of data\n' +
  'stored on your computer or mobile device by your web browser. We use Cookies to enhance the performance and functionality\n' +
  'of our app but are non-essential to their use. However, without these cookies, certain functionality like videos may become\n' +
  'unavailable or you would be required to enter your login details every time you visit the app as we would not be able to\n' +
  'remember that you had logged in previously. Most web browsers can be set to disable the use of Cookies. However, if you\n' +
  'disable Cookies, you may not be able to access functionality on our website correctly or at all. We never place Personally\n' +
  'Identifiable Information in Cookies.</p>\n' +
  '<h3>Changes To Our Terms & Conditions</h3>\n' +
  '<p>You acknowledge and agree that <?=VIP Billionaires;?> may stop (permanently or temporarily) providing the Service (or any\n' +
  'features within the Service) to you or to users generally at <?=VIP Billionaires;?>’s sole discretion, without prior notice to you.\n' +
  'You may stop using the Service at any time. You do not need to specifically inform <?=VIP Billionaires;?> when you stop using\n' +
  'the Service. You acknowledge and agree that if <?=VIP Billionaires;?> disables access to your account, you may be prevented\n' +
  'from accessing the Service, your account details or any files or other materials which is contained in your account.</p>\n' +
  '<p>If we decide to change our Terms & Conditions, we will post those changes on this page, and/or update the Terms & Conditions\n' +
  'modification date below.</p>\n' +
  '<h3>Modifications to Our app</h3>\n' +
  '<p>VIP Billionaires reserves the right to modify, suspend or discontinue, temporarily or permanently, the app or any service to which\n' +
  'it connects, with or without notice and without liability to you.</p>\n' +
  '<h3>Updates to Our app</h3>\n' +
  '<p>VIP Billionaires may from time to time provide enhancements or improvements to the features/ functionality of the app, which\n' +
  'may include patches, bug fixes, updates, upgrades and other modifications ("Updates").\n' +
  'Updates may modify or delete certain features and/or functionalities of the app. You agree that VIP Billionaires has no obligation\n' +
  'to (i) provide any Updates, or (ii) continue to provide or enable any particular features and/or functionalities of the app to you.</p>\n' +
  '<p>You further agree that all Updates will be (i) deemed to constitute an integral part of the app, and (ii) subject to the terms and\n' +
  'conditions of this Agreement.</p>\n' +
  '<h3>Third-Party Services</h3>\n' +
  '<p>We may display, include or make available third-party content (including data, information, applications and other products\n' +
  'services) or provide links to third-party websites or services ("Third- Party Services").</p>\n' +
  '<p>You acknowledge and agree that VIP Billionaires shall not be responsible for any Third-Party Services, including their accuracy,\n' +
  'completeness, timeliness, validity, copyright compliance, legality, decency, quality or any other aspect thereof. VIP Billionaires\n' +
  'does not assume and shall not have any liability or responsibility to you or any other person or entity for any Third-Party\n' +
  'Services.</p>\n' +
  '<p>Third-Party Services and links thereto are provided solely as a convenience to you and you access and use them entirely at\n' +
  'your own risk and subject to such third parties\' terms and conditions.</p>\n' +
  '<h3>Term and Termination</h3>\n' +
  '<p>This Agreement shall remain in effect until terminated by you or VIP Billionaires.</p>\n' +
  '<p>VIP Billionaires may, in its sole discretion, at any time and for any or no reason, suspend or terminate this Agreement with or\n' +
  'without prior notice.</p>\n' +
  '<p>This Agreement will terminate immediately, without prior notice from VIP Billionaires, in the event that you fail to comply with any\n' +
  'provision of this Agreement. You may also terminate this Agreement by deleting the app and all copies thereof from your\n' +
  'computer.</p>\n' +
  '<p>Upon termination of this Agreement, you shall cease all use of the app and delete all copies of the app from your computer.\n' +
  'Termination of this Agreement will not limit any of VIP Billionaires\'s rights or remedies at law or in equity in case of breach by\n' +
  'you (during the term of this Agreement) of any of your obligations under the present Agreement.</p>\n' +
  'Copyright Infringement Notice\n' +
  '<p>If you are a copyright owner or such owner’s agent and believe any material on our app constitutes an infringement on your\n' +
  'copyright, please contact us setting forth the following information: (a) a physical or electronic signature of the copyright owner\n' +
  'or a person authorized to act on his behalf; (b) identification of the material that is claimed to be infringing; (c) your contact\n' +
  'information, including your address, telephone number, and an email; (d) a statement by you that you have a good faith belief\n' +
  'that use of the material is not authorized by the copyright owners; and (e) the a statement that the information in the notification\n' +
  'is accurate, and, under penalty of perjury you are authorized to act on behalf of the owner.</p>\n' +
  '<h3>Indemnification</h3>\n' +
  '<p>You agree to indemnify and hold VIP Billionaires and its parents, subsidiaries, affiliates, officers, employees, agents, partners\n' +
  'and licensors (if any) harmless from any claim or demand, including reasonable attorneys\' fees, due to or arising out of your: (a)\n' +
  'use of the app; (b) violation of this Agreement or any law or regulation; or (c) violation of any right of a third party.</p>\n' +
  '<h3>No Warranties</h3>\n' +
  '<p>The app is provided to you "AS IS" and "AS AVAILABLE" and with all faults and defects without warranty of any kind. To the\n' +
  'maximum extent permitted under applicable law, VIP Billionaires, on its own behalf and on behalf of its affiliates and its and their\n' +
  'respective licensors and service providers, expressly disclaims all warranties, whether express, implied, statutory or otherwise,\n' +
  'with respect to the app, including all implied warranties of merchantability, fitness for a particular purpose, title and\n' +
  'non-infringement, and warranties that may arise out of course of dealing, course of performance, usage or trade practice.\n' +
  'Without limitation to the foregoing, VIP Billionaires provides no warranty or undertaking, and makes no representation of any\n' +
  'kind that the app will meet your requirements, achieve any intended results, be compatible or work with any other software, ,\n' +
  'systems or services, operate without interruption, meet any performance or reliability standards or be error free or that any\n' +
  'errors or defects can or will be corrected.</p>\n' +
  '<p>Without limiting the foregoing, neither VIP Billionaires nor any VIP Billionaires\'s provider makes any representation or warranty\n' +
  'of any kind, express or implied: (i) as to the operation or availability of the app, or the information, content, and materials or\n' +
  'products included thereon; (ii) that the app will be uninterrupted or error-free; (iii) as to the accuracy, reliability, or currency of\n' +
  'any information or content provided through the app; or (iv) that the app, its servers, the content, or e-mails sent from or on\n' +
  'behalf of VIP Billionaires are free of viruses, scripts, trojan horses, worms, malware, timebombs or other harmful components.\n' +
  '<p>Some jurisdictions do not allow the exclusion of or limitations on implied warranties or the limitations on the applicable statutory\n' +
  'rights of a consumer, so some or all of the above exclusions and limitations may not apply to you.</p>\n' +
  '<h3>Limitation of Liability</h3>\n' +
  '<p>Notwithstanding any damages that you might incur, the entire liability of VIP Billionaires and any of its suppliers under any\n' +
  'provision of this Agreement and your exclusive remedy for all of the foregoing shall be limited to the amount actually paid by you\n' +
  'for the app.</p>\n' +
  '<p>To the maximum extent permitted by applicable law, in no event shall VIP Billionaires or its suppliers be liable for any special,\n' +
  'incidental, indirect, or consequential damages whatsoever (including, but not limited to, damages for loss of profits, for loss of\n' +
  'data or other information, for business interruption, for personal injury, for loss of privacy arising out of or in any way related to\n' +
  'the use of or inability to use the app, third-party software and/or third-party hardware used with the app, or otherwise in\n' +
  'connection with any provision of this Agreement), even if VIP Billionaires or any supplier has been advised of the possibility of\n' +
  'such damages and even if the remedy fails of its essential purpose.</p>\n' +
  '<p>Some states/jurisdictions do not allow the exclusion or limitation of incidental or consequential damages, so the above limitation\n' +
  'or exclusion may not apply to you.</p>\n' +
  '<h3>Severability</h3>\n' +
  '<p>If any provision of this Agreement is held to be unenforceable or invalid, such provision will be changed and interpreted to\n' +
  'accomplish the objectives of such provision to the greatest extent possible under applicable law and the remaining provisions\n' +
  'will continue in full force and effect.</p>\n' +
  '<p>This Agreement, together with the Privacy Policy and any other legal notices published by VIP Billionaires on the Services, shall\n' +
  'constitute the entire agreement between you and VIP Billionaires concerning the Services. If any provision of this Agreement is\n' +
  'deemed invalid by a court of competent jurisdiction, the invalidity of such provision shall not affect the validity of the remaining\n' +
  'provisions of this Agreement, which shall remain in full force and effect. No waiver of any term of this Agreement shall be\n' +
  'deemed a further or continuing waiver of such term or any other term, and VIP Billionaires."’s" failure to assert any right or\n' +
  'provision under this Agreement shall not constitute a waiver of such right or provision. YOU AND VIP Billionaires AGREE THAT\n' +
  'ANY CAUSE OF ACTION ARISING OUT OF OR RELATED TO THE SERVICES MUST COMMENCE WITHIN ONE (1) YEAR\n' +
  'AFTER THE CAUSE OF ACTION ACCRUES. OTHERWISE, SUCH CAUSE OF ACTION IS PERMANENTLY BARRED.</p>\n' +
  '<h3>Waiver</h3>\n' +
  '<p>Except as provided herein, the failure to exercise a right or to require performance of an obligation under this Agreement shall\n' +
  'not effect a party\'s ability to exercise such right or require such performance at any time thereafter nor shall be the waiver of a\n' +
  'breach constitute waiver of any subsequent breach.</p>\n' +
  '<p>o failure to exercise, and no delay in exercising, on the part of either party, any right or any power under this Agreement shall\n' +
  'operate as a waiver of that right or power. Nor shall any single or partial exercise of any right or power under this Agreement\n' +
  'preclude further exercise of that or any other right granted herein. In the event of a conflict between this Agreement and any\n' +
  'applicable purchase or other terms, the terms of this Agreement shall govern.</p>\n' +
  '<h3>Amendments to this Agreement</h3>\n' +
  '<p>VIP Billionaires reserves the right, at its sole discretion, to modify or replace this Agreement at any time. If a revision is material\n' +
  'we will provide at least 30 days\' notice prior to any new terms taking effect. What constitutes a material change will be\n' +
  'determined at our sole discretion.</p>\n' +
  '<p>By continuing to access or use our app after any revisions become effective, you agree to be bound by the revised terms. If you\n' +
  'do not agree to the new terms, you are no longer authorized to use VIP Billionaires.</p>\n' +
  '<h3>Entire Agreement</h3>\n' +
  '<p>The Agreement constitutes the entire agreement between you and VIP Billionaires regarding your use of the app and\n' +
  'supersedes all prior and contemporaneous written or oral agreements between you and VIP Billionaires.\n' +
  'You may be subject to additional terms and conditions that apply when you use or purchase other VIP Billionaires\'s services,\n' +
  'which VIP Billionaires will provide to you at the time of such use or purchase.</p>\n' +
  '<h3>Updates to Our Terms</h3>\n' +
  '<p>We may change our Service and policies, and we may need to make changes to these Terms so that they accurately reflect our\n' +
  'Service and policies. Unless otherwise required by law, we will notify you (for example, through our Service) before we make\n' +
  'changes to these Terms and give you an opportunity to review them before they go into effect. Then, if you continue to use the\n' +
  'Service, you will be bound by the updated Terms. If you do not want to agree to these or any updated Terms, you can delete\n' +
  'your account.</p>\n' +
  '<h3>Intellectual Property</h3>\n' +
  '<p>The app and its entire contents, features and functionality (including but not limited to all information, software, text, displays,\n' +
  'images, video and audio, and the design, selection and arrangement thereof), are owned by VIP Billionaires, its licensors or\n' +
  'other providers of such material and are protected by Japan and international copyright, trademark, patent, trade secret and\n' +
  'other intellectual property or proprietary rights laws. The material may not be copied, modified, reproduced, downloaded or\n' +
  'distributed in any way, in whole or in part, without the express prior written permission of VIP Billionaires, unless and except as\n' +
  'is expressly provided in these Terms & Conditions. Any unauthorized use of the material is prohibited.</p>\n' +
  '<h3>Agreement to Arbitrate</h3>\n' +
  '<p>This section applies to any dispute EXCEPT IT DOESN’T INCLUDE A DISPUTE RELATING TO CLAIMS FOR INJUNCTIVE\n' +
  'OR EQUITABLE RELIEF REGARDING THE ENFORCEMENT OR VALIDITY OF YOUR OR VIP Billionaires."’s"\n' +
  'INTELLECTUAL PROPERTY RIGHTS. The term “dispute” means any dispute, action, or other controversy between you and\n' +
  'VIP Billionaires concerning the Services or this agreement, whether in contract, warranty, tort, statute, regulation, ordinance, or\n' +
  'any other legal or equitable basis. “Dispute” will be given the broadest possible meaning allowable under law.\n' +
  'Notice of Dispute</p>\n' +
  '<p>In the event of a dispute, you or VIP Billionaires must give the other a Notice of Dispute, which is a written statement that sets\n' +
  'forth the name, address, and contact information of the party giving it, the facts giving rise to the dispute, and the relief\n' +
  'requested. You must send any Notice of Dispute via email to: info@zedinternational.net. VIP Billionaires will send any Notice of\n' +
  'Dispute to you by mail to your address if we have it, or otherwise to your email address. You and VIP Billionaires will attempt to\n' +
  'resolve any dispute through informal negotiation within sixty (60) days from the date the Notice of Dispute is sent. After sixty\n' +
  '(60) days, you or VIP Billionaires may commence arbitration.</p>\n' +
  '<h3>Binding Arbitration</h3>\n' +
  '<p>If you and VIP Billionaires don’t resolve any dispute by informal negotiation, any other effort to resolve the dispute will be\n' +
  'conducted exclusively by binding arbitration as described in this section. You are giving up the right to litigate (or participate in\n' +
  'as a party or class member) all disputes in court before a judge or jury. The dispute shall be settled by binding arbitration in\n' +
  'accordance with the commercial arbitration rules of the American Arbitration Association. Either party may seek any interim or\n' +
  'preliminary injunctive relief from any court of competent jurisdiction, as necessary to protect the party’s rights or property\n' +
  'pending the completion of arbitration. Any and all legal, accounting, and other costs, fees, and expenses incurred by the\n' +
  'prevailing party shall be borne by the non-prevailing party.</p>\n' +
  '<h3>Submissions and Privacy</h3>\n' +
  '<p>In the event that you submit or post any ideas, creative suggestions, designs, photographs, information, advertisements, data or\n' +
  'proposals, including ideas for new or improved products, services, features, technologies or promotions, you expressly agree\n' +
  'that such submissions will automatically be treated as non-confidential and non-proprietary and will become the sole property of\n' +
  'VIP Billionaires without any compensation or credit to you whatsoever. VIP Billionaires and its affiliates shall have no obligations\n' +
  'with respect to such submissions or posts and may use the ideas contained in such submissions or posts for any purposes in\n' +
  'any medium in perpetuity, including, but not limited to, developing, manufacturing, and marketing products and services using\n' +
  'such ideas.</p>\n' +
  '<h3>Promotions</h3>\n' +
  '<p>VIP Billionaires may, from time to time, include contests, promotions, sweepstakes, or other activities (“Promotions”) that require\n' +
  'you to submit material or information concerning yourself. Please note that all Promotions may be governed by separate rules\n' +
  'that may contain certain eligibility requirements, such as restrictions as to age and geographic location. You are responsible to\n' +
  'read all Promotions rules to determine whether or not you are eligible to participate. If you enter any Promotion, you agree to\n' +
  'abide by and to comply with all Promotions Rules.</p>\n' +
  '<p>Additional terms and conditions may apply to purchases of goods or services on or through the Services, which terms and\n' +
  'conditions are made a part of this Agreement by this reference.</p>\n' +
  '<h3>Typographical Errors</h3>\n' +
  '<p>In the event a product and/or service is listed at an incorrect price or with incorrect information due to typographical error, we\n' +
  'shall have the right to refuse or cancel any orders placed for the product and/or service listed at the incorrect price. We shall\n' +
  'have the right to refuse or cancel any such order whether or not the order has been confirmed and your credit card charged. If\n' +
  'your credit card has already been charged for the purchase and your order is canceled, we shall immediately issue a credit to\n' +
  'your credit card account or other payment account in the amount of the charge.</p>\n' +
  '<h3>Miscellaneous</h3>\n' +
  '<p>If for any reason a court of competent jurisdiction finds any provision or portion of these Terms & Conditions to be\n' +
  'unenforceable, the remainder of these Terms & Conditions will continue in full force and effect. Any waiver of any provision of\n' +
  'these Terms & Conditions will be effective only if in writing and signed by an authorized representative of VIP Billionaires. VIP\n' +
  'Billionaires will be entitled to injunctive or other equitable relief (without the obligations of posting any bond or surety) in the\n' +
  'event of any breach or anticipatory breach by you. VIP Billionaires operates and controls the VIP Billionaires Service from its\n' +
  'offices in Japan. The Service is not intended for distribution to or use by any person or entity in any jurisdiction or country where\n' +
  'such distribution or use would be contrary to law or regulation. Accordingly, those persons who choose to access the VIP\n' +
  'Billionaires Service from other locations do so on their own initiative and are solely responsible for compliance with local laws, if\n' +
  'and to the extent local laws are applicable. These Terms & Conditions (which include and incorporate the VIP Billionaires\n' +
  'Privacy Policy) contains the entire understanding, and supersedes all prior understandings, between you and VIP Billionaires\n' +
  'concerning its subject matter, and cannot be changed or modified by you. The section headings used in this Agreement are for\n' +
  'convenience only and will not be given any legal import.</p>\n' +
  '<h3>Disclaimer</h3>\n' +
  '<p>VIP Billionaires is not responsible for any content, code or any other imprecision.</p>\n' +
  '<p>VIP Billionaires does not provide warranties or guarantees.</p>\n' +
  '<p>In no event shall VIP Billionaires be liable for any special, direct, indirect, consequential, or incidental damages or any damages\n' +
  'whatsoever, whether in an action of contract, negligence or other tort, arising out of or in connection with the use of the Service\n' +
  'or the contents of the Service. The Company reserves the right to make additions, deletions, or modifications to the contents on\n' +
  'the Service at any time without prior notice.</p>\n' +
  '<p>The VIP Billionaires Service and its contents are provided "as is" and "as available" without any warranty or representations of\n' +
  'any kind, whether express or implied. VIP Billionaires is a distributor and not a publisher of the content supplied by third parties;\n' +
  'as such, VIP Billionaires exercises no editorial control over such content and makes no warranty or representation as to the\n' +
  'accuracy, reliability or currency of any information, content, service or merchandise provided through or accessible via the VIP\n' +
  'Billionaires Service. Without limiting the foregoing, VIP Billionaires specifically disclaims all warranties and representations in\n' +
  'any content transmitted on or in connection with the VIP Billionaires Service or on sites that may appear as links on the VIP\n' +
  'Billionaires Service, or in the products provided as a part of, or otherwise in connection with, the VIP Billionaires Service,\n' +
  'including without limitation any warranties of merchantability, fitness for a particular purpose or non-infringement of third party\n' +
  'rights. No oral advice or written information given by VIP Billionaires or any of its affiliates, employees, officers, directors,\n' +
  'agents, or the like will create a warranty. Price and availability information is subject to change without notice. Without limiting\n' +
  'the foregoing, VIP Billionaires does not warrant that the VIP Billionaires Service will be uninterrupted, uncorrupted, timely, or\n' +
  'error-free.</p>\n' +
  '<h3>Contact Us</h3>\n' +
  '<p>Don\'t hesitate to contact us if you have any questions.</p>\n' +
  '<p>● Via Email: info@zedinternational.net</p>';

export const CONTENT_TERMS_AND_CONDITIONS_JP =
  '<p>VIP Billionaires （以下「私たち」といいます。）は、VIP Billionaires の提供するサービス（以下「本サービス」といいます。）における、ユーザーについての個人情報を含む利用者情報の取扱いについて、以下のとおりプライバシーポリシー（以下「本ポリシー」といいます。）を定めます。</p>\n' +
  '\n' +
  '<h3>1.収集する利用者情報及び収集方法</h3>\n' +
  '<p>本ポリシーにおいて、「利用者情報」とは、ユーザーの識別に係る情報、通信サービス上の行動履歴、その他ユーザーまたはユーザーの端末に関連して生成または蓄積された情報であって、本ポリシーに基づきVIP Billionaires が収集するものを意味するものとします。 </br/>本サービスにおいてVIP Billionaires が収集する利用者情報は、その収集方法に応じて、以下のようなものとなります。</br/>\n' +
  '(1) ユーザーからご提供いただく情報 本サービスを利用するために、または本サービスの利用を通じてユーザーからご提供いただく情報は以下のとおりです。</br/>\n' +
  '・氏名、生年月日、性別、職業等プロフィールに関する情報</br/>\n' +
  '・メールアドレス、電話番号、住所等連絡先に関する情報</br/>\n' +
  '・クレジットカード情報、銀行口座情報、電子マネー情報等決済手段に関する情報</br/>\n' +
  '・ユーザーの肖像を含む静止画情報</br/>\n' +
  '・入力フォームその他VIP Billionaires が定める方法を通じてユーザーが入力または送信する情報</br/>\n' +
  '(2) ユーザーが本サービスの利用において、他のサービスと連携を許可することにより、当該他のサービスからご提供いただく情報</br/>\n' +
  'ユーザーが、本サービスを利用するにあたり、ソーシャルネットワーキングサービス等の他のサービスとの連携を許可した場合には、その許可の際にご同意いただいた内容に基づき、以下の情報を当該外部サービスから収集します。</br/>\n' +
  '・当該外部サービスでユーザーが利用するID</br/>\n' +
  '・その他当該外部サービスのプライバシー設定によりユーザーが連携先に開示を認めた情報</br/>\n' +
  '(3) ユーザーが本サービスを利用するにあたって、VIP Billionaires が収集する情報</br/>\n' +
  'VIP Billionaires は、本サービスへのアクセス状況やそのご利用方法に関する情報を収集することがあります。これには以下の情報が含まれます。</br/>\n' +
  '・リファラ</br/>\n' +
  '・IPアドレス</br/>\n' +
  '・サーバーアクセスログに関する情報</br/>\n' +
  '・Cookie、ADID、IDFAその他の識別子</br/>\n' +
  '(4) ユーザーが本サービスを利用するにあたって、VIP Billionaires がユーザーの個別同意に基づいて収集する情報</br/>\n' +
  'VIP Billionaires は、ユーザーが3-1に定める方法により個別に同意した場合、VIP Billionaires は以下の情報を利用中の端末から収集します。</br/>\n' +
  '・位置情報</p>\n' +
  '\n' +
  '<h3>2.利用目的</h3>\n' +
  '<p>本サービスのサービス提供にかかわる利用者情報の具体的な利用目的は以下のとおりです。</p>\n' +
  '<p>(1) 本サービスに関する登録の受付、本人確認、ユーザー認証、ユーザー設定の記録、利用料金の決済計算等本サービスの提供、維持、保護及び改善のため</br/>\n' +
  '(2) ユーザーのトラフィック測定及び行動測定のため</br/>\n' +
  '(3) 広告の配信、表示及び効果測定のため</br/>\n' +
  '(4) 本サービスに関するご案内、お問い合わせ等への対応のため</br/>\n' +
  '(5) 本サービスに関するVIP Billionaires の規約、ポリシー等（以下「規約等」といいます。）に違反する行為に対する対応のため</br/>\n' +
  '(6) 本サービスに関する規約等の変更などを通知するため</p>\n' +
  '\n' +
  '<h3>3.通知・公表または同意取得の方法、利用中止要請の方法</h3>\n' +
  '<p>3-1 以下の利用者情報については、その収集が行われる前にユーザーの同意を得るものとします。</br/>\n' +
  '・位置情報</p>\n' +
  '<p>3-2 ユーザーは、本サービスの所定の設定を行うことにより、利用者情報の全部または一部についてその収集又は利用の停止を求めることができ、この場合、VIP Billionaires は速やかに、VIP Billionaires の定めるところに従い、その利用を停止します。なお利用者情報の項目によっては、その収集または利用が本サービスの前提となるため、VIP Billionaires 所定の方法により本サービスを退会した場合に限り、VIP Billionaires はその収集又は利用を停止します。</p>\n' +
  '\n' +
  '<h3>4.外部送信、第三者提供、情報収集モジュールの有無</h3>\n' +
  '<p>4-1 本サービスでは、以下の提携先が、ユーザーの端末にCookieを保存し、これを利用して利用者情報を蓄積及び利用している場合があります。</br/>\n' +
  '(1) 提携先</br/>\n' +
  '(2) 上記提携先のプライバシーポリシーのURL</br/>\n' +
  '(3) 上記提携先のオプトアウト（無効化）URL</p>\n' +
  '<p>4-2 本サービスには以下の情報収集モジュールが組み込まれています。これに伴い、以下のとおり情報収集モジュール提供者（日本国外にある者を含みます。）への利用者情報の提供を行います。</br/>\n' +
  '(1) 情報収集モジュールの名称</br/>\n' +
  '(2) 情報収集モジュールの提供者</br/>\n' +
  '(3) 提供される利用者情報の項目</br/>\n' +
  '(4) 提供の手段・方法</br/>\n' +
  '(5) 上記提供者における利用目的</br/>\n' +
  '(6) 上記提供者における第三者提供の有無</br/>\n' +
  '(7) 上記提供者のプライバシーポリシーのURL</p>\n' +
  '\n' +
  '<h3>5.第三者提供</h3>\n' +
  '<p>VIP Billionaires は、利用者情報のうち、個人情報については、あらかじめユーザーの同意を得ないで、第三者（日本国外にある者を含みます。）に提供しません。但し、次に掲げる必要があり第三者（日本国外にある者を含みます。）に提供する場合はこの限りではありません。</br/>\n' +
  '(1) VIP Billionaires が利用目的の達成に必要な範囲内において個人情報の取扱いの全部または一部を委託する場合</br/>\n' +
  '(2) 合併その他の事由による事業の承継に伴って個人情報が提供される場合</br/>\n' +
  '(3) 第4項の定めに従って、提携先または情報収集モジュール提供者へ個人情報が提供される場合</br/>\n' +
  '(4) 国の機関もしくは地方公共団体またはその委託を受けた者が法令の定める事務を遂行することに対して協力する必要がある場合であって、ユーザーの同意を得ることによって当該事務の遂行に支障を及ぼすおそれがある場合</br/>\n' +
  '(5) その他、個人情報の保護に関する法律（以下「個人情報保護法」といいます。）その他の法令で認められる場合</p>\n' +
  '\n' +
  '<h3>6.共同利用</h3>\n' +
  '<p>VIP Billionaires は、以下のとおりユーザーの個人情報を共同利用します。</br/>\n' +
  '(1) 共同して利用される個人情報の項目</br/>\n' +
  '(2) 共同して利用する者の範囲</br/>\n' +
  '(3) 共同して利用する者の利用目的</br/>\n' +
  '(4) 個人情報の管理について責任を有する者の氏名または名称</p>\n' +
  '\n' +
  '<h3>7.個人情報の開示</h3>\n' +
  '<p>VIP Billionaires は、ユーザーから、個人情報保護法の定めに基づき個人情報の開示を求められたときは、ユーザーご本人からのご請求であることを確認の上で、ユーザーに対し、遅滞なく開示を行います（当該個人情報が存在しないときにはその旨を通知いたします。）。但し、個人情報保護法その他の法令により、VIP Billionaires が開示の義務を負わない場合は、この限りではありません。なお、個人情報の開示につきましては、手数料（1件あたり1,000円）を頂戴しておりますので、あらかじめ御了承ください。</p>\n' +
  '\n' +
  '<h3>8.個人情報の訂正及び利用停止等</h3>\n' +
  '<p>8-1 VIP Billionaires は、ユーザーから、(1)個人情報が真実でないという理由によって個人情報保護法の定めに基づきその内容の訂正を求められた場合、及び(2)あらかじめ公表された利用目的の範囲を超えて取扱われているという理由または偽りその他不正の手段により収集されたものであるという理由により、個人情報保護法の定めに基づきその利用の停止を求められた場合には、ユーザーご本人からのご請求であることを確認の上で遅滞なく必要な調査を行い、その結果に基づき、個人情報の内容の訂正または利用停止を行い、その旨をユーザーに通知します。なお、訂正または利用停止を行わない旨の決定をしたときは、ユーザーに対しその旨を通知いたします。</br/>\n' +
  '8-2VIP Billionaires は、ユーザーから、ユーザーの個人情報について消去を求められた場合、VIP Billionaires が当該請求に応じる必要があると判断した場合は、ユーザーご本人からのご請求であることを確認の上で、個人情報の消去を行い、その旨をユーザーに通知します。</br/>\n' +
  '8-3個人情報保護法その他の法令により、VIP Billionaires が訂正等または利用停止等の義務を負わない場合は、8-1および8-2の規定は適用されません。</p>\n' +
  '\n' +
  '<h3>9.お問い合わせ窓口</h3>\n' +
  '<p>ご意見、ご質問、苦情のお申出その他利用者情報の取扱いに関するお問い合わせは、下記の窓口までお願いいたします。</br/>\n' +
  '住所：〒450-0003　愛知県名古屋市中村区名駅南3丁目3-21 BIANCASA水主町2階</br/>\n' +
  'ゼンエルディーンインターナショナル株式会社</br/>\n' +
  '個人情報取扱責任者：ゼンエルディーンイスマイル</br/>\n' +
  '連絡先：Eメール  info@zedinternational.net</p>\n' +
  '\n' +
  '<h3>10.プライバシーポリシーの変更手続</h3>\n' +
  '<p>VIP Billionaires は、必要に応じて、本ポリシーを変更します。但し、法令上ユーザーの同意が必要となるような本ポリシーの変更を行う場合、変更後の本ポリシーは、VIP Billionaires 所定の方法で変更に同意したユーザーに対してのみ適用されるものとします。なお、VIP Billionaires は、本ポリシーを変更する場合には、変更後の本ポリシーの施行時期及び内容をVIP Billionaires のウェブサイト上での表示その他の適切な方法により周知し、またはユーザーに通知します。</p>\n';

export const CONTENT_USER_AGREEMENT =
  '<h1>End-User License Agreement</h1>\n' +
  '<p>Updated at 2021-09-27</p>\n' +
  '<p>VIP Billionaires hereby grants you access to (“the Website”) and invites you to purchase the services offered here.</p>\n' +
  '<h3>Definitions and key terms</h3>\n' +
  '<p>To help explain things as clearly as possible in this Eula, every time any of these terms are referenced, are strictly defined as:</p>\n' +
  '<p>● Cookie: small amount of data generated by a website and saved by your web browser. It is used to identify your browser,\n' +
  'provide analytics, remember information about you such as your language preference or login information.<br/>\n' +
  '● Company: when this policy mentions “Company,” “we,” “us,” or “our,” it refers to Zein El Dine International Co., Ltd, Tokyo,\n' +
  'Japan that is responsible for your information under this Eula.<br/>\n' +
  '● Country: where VIP Billionaires or the owners/founders of VIP Billionaires are based, in this case is Japan<br/>\n' +
  '● Service: refers to the service provided by VIP Billionaires as described in the relative terms (if available) and on this\n' +
  'platform.<br/>\n' +
  '● Third-party service: refers to advertisers, contest sponsors, promotional and marketing partners, and others who provide\n' +
  'our content or whose products or services we think may interest you.<br/>\n' +
  '● You: a person or entity that is registered with VIP Billionaires to use the Services.</p>\n' +
  '<h3>Introduction</h3>\n' +
  '<p>This End User License Agreement (the “Agreement”) is a binding agreement between you (“End User”,“you” or “your”) and Zein\n' +
  'El Dine International Co., Ltd (“Company”, “we”, “us” or “our”). This Agreement governs the relationship between you and us,\n' +
  'and your use of the Company VIP Billionaires. Throughout this Agreement, End User and Company may each be referred to as\n' +
  'a “Party” or collectively, the “Parties”.</p>\n' +
  '<p>If you are using the app on behalf of your employer or other entity (an “Organisation”) for whose benefit you utilise the app or\n' +
  'who owns or otherwise controls the means through which you utilise or access the app, then the terms “End User”, “you”, and\n' +
  '“your” shall apply collectively to you as an individual and to the Organisation. If you use, or purchase a license or to, the app on\n' +
  'behalf of an Organisation, you hereby acknowledge, warrant, and covenant that you have the authority to 1) purchase a license\n' +
  'to the app on behalf of the Organisation; 2) bind the Organisation to the terms of this Agreement.</p>\n' +
  '<p>By downloading, installing, accessing, or using the app you: (a) affirm that you have all of the necessary permissions and\n' +
  'authorisations to access and use the app; (b) if you are using the app pursuant to a license purchased by an organisation, that\n' +
  'you are authorised by that organisation to access and use the app(c) acknowledge that you have read and that you understand\n' +
  'this agreement; (d) represent that you are of sound mind and of legal age (18 years of age or older) to enter into a binding\n' +
  'agreement; and (e) accept and agree to be legally bound by the terms and conditions of this agreement.</p>\n' +
  '<p>If you do not agree to these terms, do not download, install, access, or use the software. if you have already downloaded the\n' +
  'software, delete it from your computing device.</p>\n' +
  '<p>The Application is licensed, not sold, to you by VIP Billionaires for use strictly in accordance with the terms of this Agreement.</p>\n' +
  '<h3>License</h3>\n' +
  '<p>Subject to the terms of this Agreement and, if applicable, those terms provided in the License Agreement, VIP Billionaires grants\n' +
  'you a limited, non-exclusive, perpetual, revocable, and non-transferable license to:</p>\n' +
  '<p>(a) download, install and use the Software on one (1) Computing Device per single user license that you have purchased and\n' +
  'been granted. If you have multiple Computer Devices in which you wish to use the Software, you agree to acquire a license for\n' +
  'the number of devices you intend to use;<br/>\n' +
  '(b) access, view, and use on such Computing Device the End User Provided Materials made available in or otherwise\n' +
  'accessible through the Software, strictly in accordance with this Agreement, and any other terms and conditions applicable to\n' +
  'such End User Provided Materials;<br/>\n' +
  '(c) install and use the trial version of the Software on any number of Computing Devices for a trial period of fifteen (15) unique\n' +
  'days after installation.<br/>\n' +
  '(d) receive updates and new features that become available during the one (1) year period from the date on which you\n' +
  'purchased the license to the Software.</p>\n' +
  '<h3>Restrictions</h3>\n' +
  '<p>You agree not to, and you will not permit others to:</p>\n' +
  '<p>● License, sell, rent, lease, assign, distribute, transmit, host, outsource, disclose or otherwise commercially exploit the\n' +
  'Application or make the Application available to any third party.<br/>\n' +
  '● Modify, make derivative works of, disassemble, decrypt, reverse compile or reverse engineer any part of the Application.<br/>\n' +
  '● Remove, alter or obscure any proprietary notice (including any notice of copyright or trademark) of VIP Billionaires or its\n' +
  'affiliates, partners, suppliers or the licensors of the Application.</p>\n' +
  '<h3>Intellectual Property</h3>\n' +
  '<p>All intellectual property rights, including copyrights, patents, patent disclosures and inventions (whether patentable or not),\n' +
  'trademarks service marks, trade secrets, know-how and other confidential information, trade dress, trade names, logos,\n' +
  'corporate names and domain names, together with all of the good will associated there with, derivative works and all other rights\n' +
  '(collectively, “Intellectual Property Rights”) that are part of the Software that are otherwise owned by VIP Billionaires shall\n' +
  'always remain the exclusive property of VIP Billionaires (or of its suppliers or licensors, if and when applicable). Nothing in this\n' +
  'Agreement grants you (or any Organisation) a license to VIP Billionaires."’s" Intellectual Property Rights.</p>\n' +
  '<p>You agree that this is Agreement conveys a limited license to use VIP Billionaires."’s" Intellectual Property Rights, solely as part\n' +
  'of the Software (and not independently of it), and only for the effective Term of the license granted to you hereunder. \n' +
  'Accordingly, your use of any of VIP Billionaires."’s" Intellectual Property Rights independently of the Software or outside the\n' +
  'scope of this Agreement shall be considered an infringement of VIP Billionaires."’s" Intellectual Property Rights. This shall not\n' +
  'limit, however, any claim VIP Billionaires may have for a breach of contract in the event you breach a term or condition of this\n' +
  'Agreement. You shall use the highest standard of care to safeguard all Software (including all copies thereof) from infringement,\n' +
  'misappropriation, theft, misuse or unauthorised access. Except as expressly granted in this Agreement, VIP Billionaires\n' +
  'reserves and shall retain all rights, title, and interest in the Software, including all copyrights and copyrightable subject matter,\n' +
  'trademarks and trademark able subject matter, patents and patentable subject matter, trade secrets, and other intellectual\n' +
  'property rights, registered, unregistered, granted, applied-for, or both now in existence or that may be created, relating to the\n' +
  'thereto.</p>\n' +
  '<p>You (or the Organisation, if and as applicable) shall retain ownership of all Intellectual Property Rights in and to the work\n' +
  'products that you create through or with the assistance of the Software.</p>\n' +
  '<h3>Your Suggestions</h3>\n' +
  '<p>Any feedback, comments, ideas, improvements or suggestions (collectively, "Suggestions") provided by you to VIP Billionaires\n' +
  'with respect to the Application shall remain the sole and exclusive property of VIP Billionaires.</p>\n' +
  '<p>VIP Billionaires shall be free to use, copy, modify, publish, or redistribute the Suggestions for any purpose and in any way\n' +
  'without any credit or any compensation to you.</p>\n' +
  '<h3>Modifications to Application</h3>\n' +
  'VIP Billionaires reserves the right to modify, suspend or discontinue, temporarily or permanently, the Application or any service\n' +
  'to which it connects, with or without notice and without liability to you.\n' +
  '<h3>Updates to Application</h3>\n' +
  '<p>VIP Billionaires may from time to time provide enhancements or improvements to the features/ functionality of the Application,\n' +
  'which may include patches, bug fixes, updates, upgrades and other modifications ("Updates").</p>\n' +
  '<p>Updates may modify or delete certain features and/or functionalities of the Application. You agree that VIP Billionaires has no\n' +
  'obligation to (i) provide any Updates, or (ii) continue to provide or enable any particular features and/or functionalities of the\n' +
  'Application to you.</p>\n' +
  '<p>You further agree that all Updates will be (i) deemed to constitute an integral part of the Application, and (ii) subject to the terms\n' +
  'and conditions of this Agreement.</p>\n' +
  '<h3>Term and Termination</h3>\n' +
  '<p>This Agreement shall remain in effect until terminated by you or VIP Billionaires.</p>\n' +
  '<p>VIP Billionaires may, in its sole discretion, at any time and for any or no reason, suspend or terminate this Agreement with or\n' +
  'without prior notice.</p>\n' +
  '<p>This Agreement will terminate immediately, without prior notice from VIP Billionaires, in the event that you fail to comply with any\n' +
  'provision of this Agreement. You may also terminate this Agreement by deleting the Application and all copies thereof from your\n' +
  'computer.</p>\n' +
  '<p>Upon termination of this Agreement, you shall cease all use of the Application and delete all copies of the Application from your\n' +
  'computer.</p>\n' +
  '<p>Termination of this Agreement will not limit any of VIP Billionaires."’s" rights or remedies at law or in equity in case of breach by\n' +
  'you (during the term of this Agreement) of any of your obligations under the present Agreement.</p>\n' +
  '<h3>Indemnification</h3>\n' +
  '<p>You agree to indemnify, defend and hold harmless VIP Billionaires and its officers, directors, employees, agents, affiliates,\n' +
  'successors, and assigns from and against any and all losses, damages, liabilities, deficiencies, claims, actions, judgments,\n' +
  'settlements, interest, awards, penalties, fines, costs, or expenses of whatever kind, including reasonable attorneys’ fees, arising\n' +
  'from or relating to: i) your use or misuse of the Software; ii) your failure to comply with any applicable law, regulation, or\n' +
  'government directive; iii) your breach of this Agreement; or iv) your agreement or relationship with an Organisation (if\n' +
  'applicable) or any third party. Furthermore, you agree that VIP Billionaires assumes no responsibility for the information or\n' +
  'content you submit or make available through this Software or the content that is made available to you by third parties.</p>\n' +
  '<h3>No Warranties</h3>\n' +
  '<p>The Application is provided to you "AS IS" and "AS AVAILABLE" and with all faults and defects without warranty of any kind. To\n' +
  'the maximum extent permitted under applicable law, VIP Billionaires, on its own behalf and on behalf of its affiliates and its and\n' +
  'their respective licensors and service providers, expressly disclaims all warranties, whether express, implied, statutory or\n' +
  'otherwise, with respect to the Application, including all implied warranties of merchantability, fitness for a particular purpose, title\n' +
  'and non-infringement, and warranties that may arise out of course of dealing, course of performance, usage or trade practice.\n' +
  'Without limitation to the foregoing, VIP Billionaires provides no warranty or undertaking, and makes no representation of any\n' +
  'kind that the Application will meet your requirements, achieve any intended results, be compatible or work with any other\n' +
  'software, applications, systems or services, operate without interruption, meet any performance or reliability standards or be\n' +
  'error free or that any errors or defects can or will be corrected.</p>\n' +
  '<p>Without limiting the foregoing, neither VIP Billionaires nor any VIP Billionaires."’s" provider makes any representation or\n' +
  'warranty of any kind, express or implied: (i) as to the operation or availability of the Application, or the information, content, and\n' +
  'materials or products included thereon; (ii) that the Application will be uninterrupted or error-free; (iii) as to the accuracy,\n' +
  'reliability, or currency of any information or content provided through the Application; or (iv) that the Application, its servers, the\n' +
  'content, or e-mails sent from or on behalf of VIP Billionaires are free of viruses, scripts, trojan horses, worms, malware, time\n' +
  'bombs or other harmful components.</p>\n' +
  '<p>Some jurisdictions do not allow the exclusion of or limitations on implied warranties or the limitations on the applicable statutory\n' +
  'rights of a consumer, so some or all of the above exclusions and limitations may not apply to you.</p>\n' +
  '<h3>Limitation of Liability</h3>\n' +
  '<p>Notwithstanding any damages that you might incur, the entire liability of VIP Billionaires and any of its suppliers under any\n' +
  'provision of this Agreement and your exclusive remedy for all of the foregoing shall be limited to the amount actually paid by you\n' +
  'for the Application.</p>\n' +
  '<p>To the maximum extent permitted by applicable law, in no event shall VIP Billionaires or its suppliers be liable for any special,\n' +
  'incidental, indirect, or consequential damages whatsoever (including, but not limited to, damages for loss of profits, for loss of\n' +
  'data or other information, for business interruption, for personal injury, for loss of privacy arising out of or in any way related to\n' +
  'the use of or inability to use the Application, third-party software and/or third-party hardware used with the Application, or\n' +
  'otherwise in connection with any provision of this Agreement), even if VIP Billionaires or any supplier has been advised of the\n' +
  'possibility of such damages and even if the remedy fails of its essential purpose.</p>\n' +
  '<p>Some states/jurisdictions do not allow the exclusion or limitation of incidental or consequential damages, so the above limitation\n' +
  'or exclusion may not apply to you.</p>\n' +
  '<h3>Severability</h3>\n' +
  '<p>If any provision of this Agreement is held to be unenforceable or invalid, such provision will be changed and interpreted to\n' +
  'accomplish the objectives of such provision to the greatest extent possible under applicable law and the remaining provisions\n' +
  'will continue in full force and effect.</p>\n' +
  '<h3>Waiver</h3>\n' +
  '<p>No failure to exercise, and no delay in exercising, on the part of either party, any right or any power under this Agreement shall\n' +
  'operate as a waiver of that right or power. Nor shall any single or partial exercise of any right or power under this Agreement\n' +
  'preclude further exercise of that or any other right granted herein. In the event of a conflict between this Agreement and any\n' +
  'applicable purchase or other terms, the terms of this Agreement shall govern.</p>\n' +
  '<h3>Amendments to this Agreement</h3>\n' +
  '<p>VIP Billionaires reserves the right, at its sole discretion, to modify or replace this Agreement at any time. If a revision is material\n' +
  'we will provide at least 30 days\' notice prior to any new terms taking effect. What constitutes a material change will be\n' +
  'determined at our sole discretion.</p>\n' +
  '<p>By continuing to access or use our Application after any revisions become effective, you agree to be bound by the revised\n' +
  'terms. If you do not agree to the new terms, you are no longer authorized to use the Application.</p>\n' +
  '<h3>Governing Law</h3>\n' +
  '<p>The laws of Japan, excluding its conflicts of law rules, shall govern this Agreement and your use of the Application. Your use of\n' +
  'the Application may also be subject to other local, state, national, or international laws.</p>\n' +
  '<h3>Changes to this agreement</h3>\n' +
  '<p>We reserve the exclusive right to make changes to this Agreement from time to time. Your continued access to and use of the\n' +
  'app constitutes your agreement to be bound by, and your acceptance of, the terms and conditions posted at such time. You\n' +
  'acknowledge and agree that you accept this Agreement (and any amendments thereto) each time you load, access, or use the\n' +
  'app. Therefore, we encourage you to review this Agreement regularly.</p>\n' +
  '<p>If, within thirty (30) days of us posting changes or amendments to this Agreement, you decide that you do not agree to the\n' +
  'updated terms, you may withdraw your acceptance to the amended terms by providing us with written notice of your withdrawal.\n' +
  'Upon providing us with the written notice of the withdrawal of your acceptance, you are no longer authorised to access or use\n' +
  'the app.</p>\n' +
  '<h3>No Employment or Agency Relationship</h3>\n' +
  '<p>No provision of this Agreement, or any part of relationship between you and VIP Billionaires, is intended to create nor shall they\n' +
  'be deemed or construed to create any relationship between you and VIP Billionaires other than that of and end user of the app\n' +
  'and services provided.</p>\n' +
  '<h3>Equitable Relief</h3>\n' +
  '<p>You acknowledge and agree that your breach of this Agreement would cause VIP Billionaires irreparable harm for which money\n' +
  'damages alone would be inadequate. In addition to damages and any other remedies to which VIP Billionaires may be entitled,\n' +
  'you acknowledge and agree that we may seek injunctive relief to prevent the actual, threatened or continued breach of this\n' +
  'Agreement.</p>\n' +
  '<h3>Headings</h3>\n' +
  '<p>The headings in this Agreement are for reference only and shall not limit the scope of, or otherwise affect, the interpretation of\n' +
  'this Agreement.</p>\n' +
  '<h3>Geographic Restrictions</h3>\n' +
  '<p>The Company is based in Japan and provided for access and use primarily by persons located in Japan, and is maintains\n' +
  'compliance with Japan laws and regulations. If you use the app from outside Japan, you are solely and exclusively responsible\n' +
  'for compliance with local laws.</p>\n' +
  '<h3>Limitation of Time to File Claims</h3>\n' +
  '<p>Any cause of action or claim you may have arising out of or relating to this agreement or the app must be commenced within\n' +
  'one (1) year after the cause of action accrues, otherwise, such cause of action or claim is permanently barred.</p>\n' +
  '<h3>Entire Agreement</h3>\n' +
  '<p>The Agreement constitutes the entire agreement between you and VIP Billionaires regarding your use of the Application and\n' +
  'supersedes all prior and contemporaneous written or oral agreements between you and VIP Billionaires.</p>\n' +
  '<p>You may be subject to additional terms and conditions that apply when you use or purchase other VIP Billionaires."’s" services,\n' +
  'which VIP Billionaires will provide to you at the time of such use or purchase.</p>\n' +
  '<h3>Contact Us</h3>\n' +
  '<p>Don\'t hesitate to contact us if you have any questions about this Agreement.</p>\n' +
  '<p>● Via Email: info@zedinternational.net</p>\n';

export const CONTENT_USER_AGREEMENT_JP =
  '<h1>エンドユーザーライセンス契約</h1>\n' +
  '<p>更新日：2021-09-27<p>\n' +
  '<p>VIP Billionairesは、ここにあなたに(以下「本ウェブサイト」)へのアクセスを許可し、ここで提供されるサービスの購入を招待します。</p>\n' +
  '\n' +
  '<h3>定義と主要用語</h3>\n' +
  '<p>このエンドユーザーライセンス契約では、できるだけ明確に説明するために、下記の用語が使用されるたびに、以下の通り厳密に定義づけされています。<br/>\n' +
  'クッキー（Cookie）：ウェブサイトで生成され、お客様のウェブブラウザに保存される少量のデータです。これは、お客様のブラウザを識別するために使用されます。<br/>\n' +
  'クッキー：ウェブサイトによって生成され、お客様のブラウザによって保存される少量のデータ。<br/>\n' +
  '会社：本ポリシーで「会社」、「私たち」、「私たちの」と記載されている場合、それはこのエンドユーザーライセンス契約に基づいてあなたの情報に責任を持つ、日本の名古屋にあるゼンエルディーンインターナショナル株式会社を指します。<br/>\n' +
  '国：VIP Billionaires または VIP Billionaires の所有者/創設者が拠点を置く国を指し、ここでは日本を指します。<br/>\n' +
  'サービス: VIP Billionaires が提供するサービスを指し、関連する条件（ある場合）や本プラットフォームに記載されています。<br/>\n' +
  'サービス：VIP Billionaires が提供するサービスを指します。<br/>\n' +
  '第三者サービス：広告主、コンテストのスポンサー、プロモーションやマーケティングのパートナー、その他当社のコンテンツを提供したり、当社が製品やサービスを提供したりする者を指す。<br/>\n' +
  '第三者サービス：広告主、コンテストのスポンサー、プロモーションやマーケティングのパートナー、その他当社のコンテンツを提供したり、当社がお客様の興味を引くと思われる製品やサービスを提供する者を指します。<br/>\n' +
  'お客様：本サービスを利用するために VIP Billionaires に登録された個人または法人を指します。</p>\n' +
  '\n' +
  '<h3>はじめに</h3>\n' +
  '<p>このエンドユーザーライセンス契約（以下、「本契約」）は、あなた（以下、「エンドユーザー」、「あなた」または「あなたの」）とゼンエルディーンインターナショナル株式会社（以下、「当社」、「当社」、「私たち」または「私たちの」）との間の拘束力のある契約です。本契約は、お客様と当社の関係を規定するものです。およびお客様による VIP Billionaires の使用について規定しています。本契約において、利用者および当社はそれぞれ「当事者」と呼ばれることがあります。当事者」または総称して「当事者」と呼ぶことがあります。<br/>\n' +
  'お客様が雇用主またはその他の団体（「組織」）のために本アプリを利用している場合、その利益のために本アプリを利用している、または お客様が本アプリを利用する利益のために本アプリを利用している場合、またはお客様が本アプリを利用またはアクセスする手段を所有またはその他の方法で管理している場合、「エンドユーザー」、「お客様」および「お客様の」という用語は、お客様に総称して適用されます。「お客様の」という用語は、個人としてのお客様と組織に総称して適用されるものとします。お客様が組織を代表して本アプリを使用する場合、または本アプリのライセンスを購入する場合 お客様が組織を代表して本アプリを使用またはライセンスを購入する場合、お客様はここに、1）組織を代表して本アプリのライセンスを購入する権限を有することを認め、保証し、誓約します。<br/>2) 組織を本契約の条件に拘束する。\n' +
  'お客様は、本アプリをダウンロード、インストール、アクセス、または使用することにより、以下のことを行います。<br/>(a) 本アプリへのアクセスおよび使用に必要なすべての許可および権限を有していることを確認します。<br/>\n' +
  '(b) お客様が組織によって購入されたライセンスに基づいてアプリを使用している場合、お客様はその組織からアプリを使用する権限を与えられていること。<br/>\n' +
  '(c) お客様が本契約を読み、理解していることを認めること。<br/>(d) お客様が健全な精神の持ち主であり、拘束力のある契約を締結できる法的年齢（18歳以上）であることを表明すること。<br/>(e) 本契約の条件に法的に拘束されることを受け入れ、同意すること。\n' +
  'これらの条件に同意されない場合は、本ソフトウェアをダウンロード、インストール、アクセス、または使用しないでください。ソフトウェアをダウンロードしている場合は、お客様のコンピュータから削除してください。<br/>\n' +
  '本アプリケーションは、本契約の条項に厳密に従った使用のために、VIP Billionaires によってお客様にライセンス供与されるものであり、販売されるものではありません。</p>\n' +
  '\n' +
  '<h3>ライセンス</h3>\n' +
  '<p>VIP Billionaires は、本契約の条項および該当する場合はライセンス契約に記載された条項に従い、以下を許諾します。お客様には、以下の限定的、非独占的、永続的、取消可能、かつ譲渡不能なライセンスを付与します。<br/>\n' +
  '(a) 利用者が購入し、付与されたシングルユーザーライセンスごとに、1台の端末上で本ソフトウェアをダウンロード、インストール、使用すること。(a)お客様が購入し付与されたシングルユーザーライセンスごとに、1台のコンピュータデバイス上で本ソフトウェアをダウンロード、インストールおよび使用する。お客様が本ソフトウェアの使用を希望するコンピュータデバイスを複数お持ちの場合、お客様は以下の数のライセンスを取得することに同意するものとします。お客様は、使用しようとする機器の数だけライセンスを取得することに同意するものとします。<br/>\n' +
  '(b) 当該コンピューティングデバイス上で、本契約および当該エンドユーザー提供マテリアルに適用されるその他の条件に厳密に従い、本ソフトウェアにおいて利用可能な、またはその他の方法でアクセス可能なエンドユーザー提供マテリアルにアクセスし、閲覧し、使用すること。エンドユーザー提供マテリアル。<br/>\n' +
  '(c) インストール後、15日間の試用期間中に、任意の台数のコンピュータデバイスに本ソフトウェアの試用版をインストールして使用すること。<br/>\n' +
  '(d) お客様が本ソフトウェアのライセンスを購入された日から1年間に利用可能となるアップデートおよび新機能を受け取ること。</p>\n' +
  '\n' +
  '<h3>制限事項</h3>\n' +
  '<p>お客様は、以下の行為を行わないことに同意し、また他者に許可しないものとします。\n' +
  'お客様は、本アプリケーションをライセンス、販売、レンタル、リース、割り当て、配布、送信、ホスト、アウトソース、開示、またはその他の方法で商業的に利用しないことに同意します。<br/>\n' +
  'お客様は、本アプリケーションのライセンス供与、販売、レンタル、リース、割り当て、配布、送信、ホスト、アウトソース、開示、その他の商業的利用、または本アプリケーションの第三者への提供を行わないことに同意します。<br/>\n' +
  '本アプリケーションのいかなる部分も修正、派生物の作成、逆アセンブル、解読、逆コンパイル、リバースエンジニアリングすること。<br/>\n' +
  'VIP Billionaires、その関連会社、パートナー、サプライヤー、またはライセンスの所有権表示（著作権または商標の表示を含む）を削除、変更、または不明瞭にすること。<br/>\n' +
  'VIP Billionaires、その関連会社、パートナー、サプライヤー、または本アプリのライセンサーの所有権表示（著作権や商標の表示を含む）を削除、変更、不明瞭にすること。</p>\n' +
  '\n' +
  '<h3>知的財産について</h3>\n' +
  '<p>著作権、特許権、特許開示、発明（特許権の有無を問わない）を含むすべての知的財産権、商標、サービスマーク、トレードシークレット、ノウハウ、その他の機密情報、トレードドレス、商号、ロゴ、企業名、ドメイン名、およびそれらに関連する善意、派生物、その他すべての権利商標サービスマーク、企業秘密、ノウハウ、その他の機密情報、トレードドレス、商号、ロゴ、企業名、ドメイン名、それに付随するすべての善意、派生物、その他のすべての権利（総称して「知的財産権」）を含む、「ソフトウェア」の一部であり、VIP Billionairesが所有するすべての知的財産権は、常にVIP Billionaires（または該当する場合はそのサプライヤーやライセンサー）の独占的な財産であるものとします。本契約のいかなる条項も、お客様（またはいかなる組織）にVIP Billionairesの知的財産権のライセンスを付与するものではありません。<br/>\n' +
  'お客様は、本契約がVIP Billionairesの知的財産権を、本ソフトウェアの一部としてのみ（独立してではなく）、本契約に基づいてお客様に付与されたライセンスの有効期間中に限り、使用するための限定的なライセンスを付与するものであることに同意するものとします。\n' +
  'したがって、お客様が「ソフトウェア」とは別に、または本契約の範囲外で「VIP Billionaires」の知的財産権を使用することは、「VIP Billionaires」の知的財産権の侵害とみなされます。ただし、お客様が本契約の条項または条件に違反した場合に、契約違反を理由に VIP Billionaires が有する請求を制限するものではありません。お客様は、最高水準の注意を払って、すべての「ソフトウェア」（そのすべてのコピーを含む）を侵害、流用、盗難、誤用、または不正アクセスから保護するものとします。本契約で明示的に許諾されている場合を除き、VIP Billionaires は本ソフトウェアに関するすべての権利、権原、利益を留保し、これにはすべての著作権および著作権対象物、商標および商標登録可能な対象物、特許および特許登録可能な対象物、企業秘密、その他の知的財産権の登録、未登録、付与、申請、またはその両方、および現在存在している、または作成される可能性のある、本ソフトウェアに関連するものを含みます。<br/>\n' +
  'お客様（または該当する場合は組織）は、お客様が本サービスを通じてまたは本サービスを利用して作成した作品およびその製品に関するすべての知的財産権の所有権を保持するものとします。</p>\n' +
  '\n' +
  '<h3>ご提案</h3>\n' +
  '<p>本アプリケーションに関して、お客様からVIP Billionairesに提供されたフィードバック、コメント、アイデア、改善、提案（以下、総称して「提案」といいます）は、VIP Billionairesの完全かつ排他的な財産となります。<br/>\n' +
  'VIP Billionairesは、目的や方法を問わず、お客様への評価や報酬を必要とせず、ご提案を自由に使用、コピー、修正、公開、再配布することができます。</p>\n' +
  '\n' +
  '<h3>アプリケーションの変更</h3>\n' +
  '<p>VIP Billionairesは、予告の有無にかかわらず、お客様に責任を負うことなく、本アプリケーションまたは本アプリケーションが接続されているサービスを一時的または永続的に変更または中断する権利を有します。</p>\n' +
  '\n' +
  '<h3>アプリケーションの更新</h3>\n' +
  '<p>VIP Billionairesは、本アプリケーションの機能を強化または改善することがあります。<br/>\n' +
  'これにはパッチ、バグフィックス、アップデート、アップグレード、その他の変更が含まれます（以下「アップデート」といいます）。<br/>\n' +
  'アップデートは本アプリの特定の機能を変更または削除する場合があります。お客様は、VIP Billionaires が(i)アップデートを提供する義務を負わないこと、(ii)本アプリの特定の機能をお客様に提供し続ける、または有効にする義務を負わないことに同意するものとします。<br/>\n' +
  'お客様はさらに、すべての「アップデート」が(i)本アプリケーションの不可欠な一部を構成するものとみなされ、(ii)本契約の条件に従うことに同意されたものとします。</p>\n' +
  '\n' +
  '<h3>契約期間と解除</h3>\n' +
  '<p>本契約は、お客様またはVIP Billionairesが終了するまで有効とします。\n' +
  'VIP Billionairesは、独自の判断により、いつでも、いかなる理由でも、事前の通知の有無にかかわらず、本契約を中断または終了することができます。予告なしに本契約を中断または終了することができます。<br/>\n' +
  '本契約は、お客様が本契約のいずれかの条項に従わなかった場合、VIP Billionaires からの事前の通知なしに直ちに終了します。また、お客様は、本アプリケーションおよびそのすべてのコピーをお客様のコンピュータから削除することにより、本契約を終了することができます。<br/>\n' +
  '本契約の終了に伴い、お客様は本アプリの使用をすべて中止し、本アプリのすべてのコピーをお客様のコンピューターから削除するものとします。<br/>\n' +
  '本契約の終了は、お客様が本契約に基づく義務のいずれかに違反した場合（本契約の期間中に）、法律上または衡平法上のVIP Billionairesの権利または救済手段を制限するものではありません。</p>\n' +
  '\n' +
  '<h3>補償</h3>\n' +
  '<p>お客様は、すべての損失、損害、負債、欠陥、請求、訴訟、判決、和解、利子、賞、罰則、罰金、コスト、または損失、損害、負債、欠陥、請求、訴訟、判決、和解、利息、賞、罰則、罰金、コスト、または以下に関連して発生するあらゆる種類の費用（合理的な弁護士費用を含む）について、VIP Billionairesとその役員、取締役、従業員、代理人、関連会社、後継者、および譲受人を守り、無害に保つことに同意します。<br/>i) お客様による本ソフトウェアの使用または誤使用、<br/>ii) お客様による適用される法律、規制、または政府の指示の不遵守、<br/>iii) お客様による本ソフトウェアの使用または誤使用の違反、またはこれらに関連するもの。<br/>iv）組織（該当する場合）または第三者との合意または関係。<br/>さらに、利用者は、本サイトを通じて利用者が提出または利用可能にした情報またはコンテンツについて、VIP Billionaires が一切の責任を負わないことに同意するものとします。</p>\n' +
  '\n' +
  '<h3>保証なし</h3>\n' +
  '<p>本アプリは、「現状有姿」で提供されており、いかなる種類の保証もなく、すべての欠点および欠陥がある状態で提供されています。適用法で認められている最大限の範囲で、VIP Billionaires は、自身、その関連会社、そのライセンサー、サービスプロバイダーを代表して、明示的、黙示的、法定、その他を問わず、本アプリに関するすべての保証を明示的に放棄します。これには、商品性、特定目的への適合性、権原、権利を侵害していないことに関するすべての黙示的な保証、取引の過程、履行の過程、使用、取引慣行から生じる保証が含まれます。<br/>\n' +
  '上記に限定されず、VIP Billionaires は、本アプリがお客様の要件を満たし、意図した結果を達成し、他のソフトウェア、システム、サービスと互換性があり、機能し、中断することなく動作し、性能や信頼性の基準を満たし、エラーがなく、エラーや欠陥が修正されることを保証または約束せず、いかなる種類の表明も行いません。また、エラーや欠陥が修正されることもありません。<br/>\n' +
  '上記を制限することなく、VIP Billionaires および VIP Billionaires の提供者は、明示的または黙示的を問わず、いかなる種類の表明または保証も行いません。(i)本アプリの動作や利用可能性、またはそこに含まれる情報、コンテンツ、素材や製品に関すること、(ii)本アプリが中断されない、またはエラーがないこと、(iii)本アプリを通じて提供される情報やコンテンツの正確性、信頼性、または通貨に関すること、または(iv)本アプリ、そのサーバー、コンテンツ、またはVIP BillionairesからもしくはVIP Billionairesを代表して送信される電子メールに、ウイルス、スクリプト、トロイの木馬、ワーム、マルウェア、タイムボム、その他の有害な要素がないこと。<br/>\n' +
  '管轄区域によっては、黙示的な保証の除外や制限、消費者の適用される法定権利の制限を認めていないため、上記の除外や制限の一部または全部がお客様に適用されない場合があります。</p>\n' +
  '\n' +
  '<h3>責任の制限</h3>\n' +
  '<p>お客様が被る可能性のある損害にかかわらず、本契約のいずれかの条項に基づく VIP Billionaires およびその供給者の全責任、および上記のすべてに対するお客様の唯一の救済手段は、本アプリに対してお客様が実際に支払った金額に限定されます。<br/>\n' +
  '適用法で認められている最大限の範囲で、いかなる場合も、VIP Billionaires Billionaires およびその供給者は、特別、偶発的、間接的、または結果的な損害（利益の損失、データやその他の情報の損失、事業の中断、個人的な傷害、プライバシーの喪失などを含むがこれらに限定されない）について責任を負いません。本アプリの使用または使用できないこと、第三者のソフトウェアおよび／または本アプリとともに使用される第三者のハードウェア、あるいは本契約のいずれかの規定に関連して生じるプライバシーの喪失）については、たとえVIP Billionaires またはサプライヤーがそのような損害の可能性を知らされていたとしても、また、救済策が本質的な目的を果たせなかったとしても、一切の責任を負いません。<br/>\n' +
  '州や司法権によっては、偶発的または結果的な損害の除外または制限を認めていないため、上記の制限または除外が 上記の制限はお客様に適用されない場合があります。</p>\n' +
  '\n' +
  '<h3>分離可能性</h3>\n' +
  '<p>本契約のいずれかの条項が執行不能または無効であると判断された場合、当該条項は適用法の下で可能な限りその目的を達成するように変更および解釈され、残りの条項は完全な効力を持ち続けるものとします。\n' +
  '本規約は、プライバシーポリシーおよびVIP Billionaires が本サービス上で公表するその他の法的通知とともに、本サービスに関するお客様とVIP Billionaires との間の完全な合意を構成するものとします。本規約のいずれかの条項が管轄裁判所によって無効と判断された場合でも、当該条項の無効性は本規約の残りの条項の有効性に影響を与えず、それらは完全に効力を有するものとします。本契約のいずれかの条項を放棄しても、その条項またはその他の条項をさらにまたは継続して放棄したとはみなされません。また、VIP Billionaires が本契約のいずれかの権利または条項を主張しなかったとしても、そのような権利または条項を放棄したとはみなされません。お客様と VIP Billionaires は、本サービスに起因または関連するいかなる訴因も、訴因発生後 1 年以内に開始しなければならないことに同意します。そうでない場合は、かかる訴訟原因は永久に禁止されます。<br/>\n' +
  '権利放棄\n' +
  '本契約に規定されている場合を除き、本契約に基づく権利の行使または義務の履行を要求しなかった場合でも、その後いつでも当該権利を行使したり、当該履行を要求したりする当事者の能力に影響を与えるものではなく、また、ある違反を放棄した場合でも、その後の違反を放棄したことにはならないものとします。<br/>\n' +
  '本契約に基づくいかなる権利または権限についても、いずれかの当事者が行使を怠ったり、行使を遅らせたりしても、その権利または権限を放棄したことにはなりません。また、本契約に基づく権利または権限を単独または部分的に行使した場合、当該権利または本契約で認められているその他の権利のさらなる行使を妨げるものではありません。本契約と、適用される購入条件またはその他の条件との間に矛盾がある場合は、本契約の条件が優先されます。</p>\n' +
  '\n' +
  '<h3>本規約の修正</h3>\n' +
  '<p>VIP Billionaires は、独自の判断により、いつでも本規約を修正または変更する権利を有します。修正が重要である場合 は、新しい条件が有効になる前に、少なくとも30日前に通知します。何が重要な変更であるかは 当社の独自の裁量により決定されます。\n' +
  '改訂が有効になった後も、当社のアプリへのアクセスまたは使用を継続することにより、お客様は改訂された条件に拘束されることに同意するものとします。もしお客様が 新しい規約に同意しない場合は、もはやVIP Billionaires を使用する権限はありません。</p>\n' +
  '\n' +
  '<h3>準拠法</h3>\n' +
  '<p>本契約およびお客様による本アプリのご利用に関しては、抵触法を除き、日本国法が適用されます。お客様の本アプリケーションのご利用には また、お客様の本アプリケーションのご利用には、他の地域、州、国、または国際法が適用される場合があります。</p>\n' +
  '\n' +
  '<h3>本契約の変更</h3>\n' +
  '<p>当社は、本契約を随時変更する独占的な権利を有します。お客様がアプリに継続してアクセスし、使用することで、その時点で掲載されている条件に拘束されることに同意し、これを受諾したものとみなされます。お客様は、アプリをロード、アクセス、または使用するたびに、本契約（およびその修正）を受諾することを認識し、同意します。したがって、お客様には本契約を定期的に確認することをお勧めします。<br/>\n' +
  '当社が本契約の変更または修正を掲載してから30日以内に、お客様が更新された条件に同意しないと判断した場合、お客様は当社に書面による撤回の通知を行うことにより、修正された条件への同意を撤回することができます。<br/>\n' +
  '書面による承諾撤回の通知を当社に提供した時点で、お客様は本アプリにアクセスまたは使用する権限を失います。</p>\n' +
  '\n' +
  '<h3>雇用または代理店関係なし</h3>\n' +
  '<p>本契約のいかなる条項、またはお客様とVIP Billionairesとの関係のいかなる部分も、お客様とVIP Billionairesとの間にアプリのエンドユーザー以外の関係を構築することを意図したものではなく、またそのような関係を構築するものでもありません。</p>\n' +
  '\n' +
  '<h3>衡平法上の救済</h3>\n' +
  '<p>お客様は、お客様が本契約に違反した場合、金銭的な損害賠償だけでは不十分な回復不能な損害を VIP Billionaires に与えることを認め、同意するものとします。損害賠償およびVIP Billionairesが受けることのできるその他の救済措置に加えて、お客様は、本契約の実際の違反、脅迫、または継続的な違反を防ぐために、差し止めによる法的救済措置を求めることができることを認め、同意するものとします。</p>\n' +
  '\n' +
  '\n' +
  '<h3>見出し</h3>\n' +
  '<p>本契約の見出しは参考のためのものであり、本契約の解釈の範囲を限定したり、その他の点で影響を与えるものではありません。</p>\n' +
  '\n' +
  '<h3>地理的制限</h3>\n' +
  '<p>当社は、日本に拠点を置き、主に日本に居住する人によるアクセスおよび使用のために提供されており、日本の法令を遵守しています。お客様が日本国外から本アプリをご利用になる場合は、お客様ご自身が単独かつ排他的に責任を負うことになります。本アプリを日本国外からご利用になる場合は、お客様ご自身の責任において、現地の法律を遵守してください。</p>\n' +
  '\n' +
  '<h3>訴えの提起期間の制限</h3>\n' +
  '<p>本契約または本アプリに起因または関連するお客様の訴訟または請求は、その原因が発生してから1年以内に開始しなければなりません。そうでない場合は、当該訴訟または請求は永久に禁止されます。</p>\n' +
  '\n' +
  '<h3>完全合意</h3>\n' +
  '<p>本契約は、本アプリケーションの使用に関するお客様とVIP Billionairesとの間の完全な合意を構成し、お客様とVIP Billionairesとの間で交わされたすべての事前の書面または口頭での合意に優先します。<br/>\n' +
  'お客様は、他のVIP Billionairesのサービスを使用または購入する際に適用される追加の条件に従うことができます。追加の条件はそのような使用または購入時にVIP Billionairesがお客様に提供します。</p>\n' +
  '\n' +
  '<h3>お問い合わせ</h3>\n' +
  '<p>本契約についてご不明な点がございましたら、お気軽にお問い合わせください。\n' +
  'メールでのお問い合わせ: info@zedinternational.net</p>\n';
